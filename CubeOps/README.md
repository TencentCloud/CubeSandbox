...
| `DATABASE_URL` | *(optional)* | Full connection URL (`mysql://` or `postgres://`). The password inside the URL must be percent-encoded (`p#a?b/c` → `p%23a%3Fb%2Fc`); prefer the `CUBE_SANDBOX_MYSQL_{HOST,PORT,USER,PASSWORD,DB}` fields when it contains URL-reserved characters (the URL wins when both are set). One of the two forms is required. For postgres, `?sslmode=…` is honored; other query parameters are rejected. |
