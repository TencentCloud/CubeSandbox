...
| `DATABASE_URL` | *(optional)* | Full connection URL (`mysql://` or `postgres://`). Alternatively the DB can be set via the `CUBE_SANDBOX_MYSQL_{HOST,PORT,USER,PASSWORD,DB}` fields; the URL wins when both are set. One of the two forms is required. |
