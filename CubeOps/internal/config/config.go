...
type TerminalConfig struct {
	...
	StdinQueueFrames      int      `yaml:"stdin_queue_frames"`
	StdoutPendingBytes    int      `yaml:"stdout_pending_bytes"`
...
func overrideFromEnv(cfg *Config) {
	...
	overrideTerminalInt(&cfg.Terminal.IdleTimeoutMinutes, "CUBE_TERMINAL_IDLE_TIMEOUT_MINUTES")
	overrideTerminalInt(&cfg.Terminal.MaxLifetimeHours, "CUBE_TERMINAL_MAX_LIFETIME_HOURS")
	overrideTerminalInt(&cfg.Terminal.ReconnectGraceSeconds, "CUBE_TERMINAL_RECONNECT_GRACE_SECONDS")
	overrideTerminalInt(&cfg.Terminal.ReplayBufferBytes, "CUBE_TERMINAL_REPLAY_BUFFER_BYTES")
	overrideTerminalInt(&cfg.Terminal.MaxFrameBytes, "CUBE_TERMINAL_MAX_FRAME_BYTES")
	overrideTerminalInt(&cfg.Terminal.StdinQueueFrames, "CUBE_TERMINAL_STDIN_QUEUE_FRAMES")
	overrideTerminalInt(&cfg.Terminal.StdoutPendingBytes, "CUBE_TERMINAL_STDOUT_PENDING_BYTES")
...
func validateTerminalConfig(cfg TerminalConfig) error {
	positive := map[string]int{
		...
		"max_lifetime_hours":        cfg.MaxLifetimeHours,
		"replay_buffer_bytes":       cfg.ReplayBufferBytes,
		"max_frame_bytes":           cfg.MaxFrameBytes,
		"stdin_queue_frames":        cfg.StdinQueueFrames,
		"stdout_pending_bytes":      cfg.StdoutPendingBytes,
	...
	if cfg.StdoutPendingBytes < cfg.MaxFrameBytes {
		return fmt.Errorf("terminal.stdout_pending_bytes must be at least terminal.max_frame_bytes")
	}
