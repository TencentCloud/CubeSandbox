...
func (h *SDKHandler) CreateTerminalSession(c *gin.Context) {
	...
	slog.Info("terminal.session.grant_issued", "session_id", issued.sessionID, "sandbox_id", target.sandboxID, "container_id", target.containerID, "principal", c.GetString("username"))
...
func (h *SDKHandler) ProxyTerminal(c *gin.Context) {
	...
	defer lease.release()
	slog.Info("terminal.session.open", "session_id", lease.sessionID, "sandbox_id", lease.target.sandboxID, "container_id", lease.target.containerID, "principal", lease.principal)
	if err := h.terminal.relay(c.Request, connection, lease); err != nil {
		slog.Warn("terminal.session.close", "session_id", lease.sessionID, "sandbox_id", lease.target.sandboxID, "container_id", lease.target.containerID, "principal", lease.principal, "reason", err)
	} else {
		slog.Info("terminal.session.close", "session_id", lease.sessionID, "sandbox_id", lease.target.sandboxID, "container_id", lease.target.containerID, "principal", lease.principal, "reason", "normal")
	}
}
