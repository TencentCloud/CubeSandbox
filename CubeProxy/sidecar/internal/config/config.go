...
// Config holds the sidecar's runtime parameters. The shape is intentionally
// flat — every field maps to a single env var — so the operator can wire it
// up via systemd EnvironmentFile= without a YAML parser dependency.
type Config struct {
	...
	// internal sub-location). Bind to loopback in same-host deployments.
	ListenAddr string
...
func Default() *Config {
	return &Config{
		...
		// `common.http_port`). Override via CUBE_SIDECAR_CUBEMASTER_URL.
...
func Load() (*Config, error) {
	...
	if v := os.Getenv("CUBE_SIDECAR_REDIS_ADDR"); v != "" {
		c.RedisAddr = v
	}
	if v := os.Getenv("CUBE_SIDECAR_REDIS_PASSWORD"); v != "" {
		c.RedisPassword = v
	}
	if v := os.Getenv("CUBE_SIDECAR_REDIS_DB"); v != "" {
		n, err := strconv.Atoi(v)
		if err != nil {
			addErr("CUBE_SIDECAR_REDIS_DB", err)
		} else {
			c.RedisDB = n
		}
	}
	...
	if v := os.Getenv("CUBE_SIDECAR_ADMIN_TOKEN"); v != "" {
		c.CubeAdminToken = v
	}
	if v := os.Getenv("CUBE_SIDECAR_CUBEMASTER_URL"); v != "" {
		c.CubeMasterURL = v
	}
	if v := os.Getenv("CUBE_SIDECAR_LISTEN_ADDR"); v != "" {
		c.ListenAddr = v
	}
	if v := os.Getenv("CUBE_SIDECAR_DEFAULT_IDLE_TIMEOUT"); v != "" {
		if d, err := time.ParseDuration(v); err != nil {
			addErr("CUBE_SIDECAR_DEFAULT_IDLE_TIMEOUT", err)
		} else {
			c.DefaultIdleTimeout = d
		}
	}
	if v := os.Getenv("CUBE_SIDECAR_LAST_ACTIVE_POLL"); v != "" {
		if d, err := time.ParseDuration(v); err != nil {
			addErr("CUBE_SIDECAR_LAST_ACTIVE_POLL", err)
		} else {
			c.LastActivePoll = d
		}
	}
	if v := os.Getenv("CUBE_SIDECAR_IDLE_SWEEP_INTERVAL"); v != "" {
		if d, err := time.ParseDuration(v); err != nil {
			addErr("CUBE_SIDECAR_IDLE_SWEEP_INTERVAL", err)
		} else {
			c.IdleSweepInterval = d
		}
	}
	if v := os.Getenv("CUBE_SIDECAR_BOOTSTRAP_WARMUP"); v != "" {
		if d, err := time.ParseDuration(v); err != nil {
			addErr("CUBE_SIDECAR_BOOTSTRAP_WARMUP", err)
		} else {
			c.BootstrapWarmup = d
		}
	}
	if v := os.Getenv("CUBE_SIDECAR_STATE_LOCK_TTL"); v != "" {
		if d, err := time.ParseDuration(v); err != nil {
			addErr("CUBE_SIDECAR_STATE_LOCK_TTL", err)
		} else {
			c.StateLockTTL = d
		}
	}
	if v := os.Getenv("CUBE_SIDECAR_CONSUMER_NAME"); v != "" {
		c.ConsumerName = v
	}
