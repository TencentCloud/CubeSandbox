...
function formatLogTime(ts: string): string {
  ...
  const time = `${p(d.getHours(), 2)}:${p(d.getMinutes(), 2)}:${p(d.getSeconds(), 2)}.${p(d.getMilliseconds(), 3)}`;
  // Prefix the date for entries from another day so cross-midnight events
  // cannot look out of order.
  const now = new Date();
  const sameDay =
    d.getFullYear() === now.getFullYear() &&
    d.getMonth() === now.getMonth() &&
    d.getDate() === now.getDate();
  return sameDay ? time : `${p(d.getMonth() + 1, 2)}-${p(d.getDate(), 2)} ${time}`;
}
...
export default function SandboxDetailPage() {
  ...
  const logs = useQuery({
    ...
    queryFn: () => sandboxApi.logs(sandboxID, { limit: 1000, tail: true }),
