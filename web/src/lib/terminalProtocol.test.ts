...
describe('terminal protocol', () => {
  it('drops a stale container when navigating to another sandbox', () => {
    expect(
      reconcileTerminalContainer('sandbox-a-main', ['sandbox-b-main', 'sandbox-b-sidecar']),
    ).toBe('sandbox-b-main');
    expect(
      reconcileTerminalContainer('sandbox-b-sidecar', ['sandbox-b-main', 'sandbox-b-sidecar']),
    ).toBe('sandbox-b-sidecar');
  });
  it('keeps the one-time grant out of the URL', () => {
    ...
    expect(url).toBe('wss://console.example/terminal/sandboxes/sandbox-a-b');
