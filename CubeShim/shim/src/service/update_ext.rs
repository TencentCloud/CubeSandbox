...
pub struct UpdateOutcome {
    ...
    /// successfully (PauseToSnapshot: MicroVM already deleted, durable snap
    /// written; Cubelet resumes by recreating from catalog).
    pub exit_shim: bool,
}
...
async fn do_pause_to_snapshot(
    ...
    infof!(log, "pause to snapshot: finished; shim will exit");
    Ok(UpdateOutcome { exit_shim: true })
}
