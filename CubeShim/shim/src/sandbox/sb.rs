...
impl SandBox {
    ...
    fn get_storages(&mut self) -> CResult<Vec<agent::Storage>> {
        ...
        let skip_virtiofs_guest_mount =
            self.app_snapshot_restore() && self.pause_resume();
    ...
    pub async fn pause_vm(&mut self) -> CResult<()> {
        // Legacy path: hardcoded pausevm directory, no CubeCow memory volume.
        // Prefer `pause_vm_to_snapshot` with Cubelet-provided destination +
        // memory_vol_url so pause artifacts share the CommitSandbox layout.
        let snapshot_path = format!("{}/{}", PAUSE_VM_SNAPSHOT_BASE, self.id);
        self.pause_vm_to_snapshot(&snapshot_path, None).await
    }
    ...
    ///
    /// On success the MicroVM is deleted (`pause2snapshot`). The shim process
    /// is left for the caller to exit (see `PauseToSnapshot` update_ext).
