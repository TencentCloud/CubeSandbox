...
def build_agent(llm, sandbox: Sandbox):
    ...
    def run_python(code: str) -> str:
        """Execute Python inside the Cube Sandbox MicroVM; return stdout + stderr.
    ...
    tools = [Tool(
        name="run_python",
        func=run_python,
        description="Execute Python code in a Cube Sandbox MicroVM with pandas/numpy/matplotlib "
                    "preinstalled. Returns stdout, with stderr (if any) delimited below it, plus "
                    "the exit code on failure. Save charts under /workspace.",
    )]
    prompt = PromptTemplate.from_template(
        "You are a data analyst. Answer the question using the tools below.\n\n"
        "Environment facts:\n"
        ...
        "per month, then use a numeric month index (0, 1, 2, ...) as the only feature.\n"
...
if __name__ == "__main__":
    ...
    with Sandbox.create(template=os.environ["CUBE_TEMPLATE_ID"], timeout=600) as sandbox:
        ...
        executor = build_agent(llm, sandbox)
