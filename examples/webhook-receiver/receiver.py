...
via webhook callbacks. Both CubeAPI and cube-lifecycle-manager (CLM) emit the
same protocol — HTTP POST with an X-Cube-Signature-256 HMAC-SHA256 header — so
this receiver works for either emitter unchanged.
...
Then configure cube-lifecycle-manager to send webhooks to this receiver:
    export CUBE_LCM_WEBHOOK_URLS="http://127.0.0.1:8081/webhook"
    export CUBE_LCM_WEBHOOK_EVENTS="*"
    export CUBE_LCM_WEBHOOK_SECRET="my-secret"   # must match WEBHOOK_SECRET above
"""
