...
演示如何通过 CubeProxy **明文 gRPC 接入端口** `9091` 访问沙箱内服务。
...
`*.cube.app` 或在 CubeProxy 上终止 TLS 时, 使用 `9091` 端口。

> **安全提示：** `9091` 是**明文** gRPC（示例使用 `grpc.insecure_channel`）。
...
| `CUBE_PROXY_GRPC_PORT` | 否 | `9091` | 明文 gRPC 监听端口 |
