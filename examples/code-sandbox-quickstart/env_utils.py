...
def _envd_init_base_url(host: str) -> str:
    """Build the /init base URL with a safe default scheme.

    HTTP is only used for the local dev sidecar (loopback). When the sidecar is
    inactive, *.cube.app traffic defaults to HTTPS so env vars and traffic
    tokens are not sent in plaintext. Set ``CUBE_ENVD_INIT_SCHEME=http`` only
    for deployments that intentionally use plaintext HTTP.
    """
    if "://" in host:
        return host.rstrip("/")
    scheme_override = os.environ.get("CUBE_ENVD_INIT_SCHEME", "").strip().lower()
    if scheme_override in ("http", "https"):
        return f"{scheme_override}://{host}"
    if _SIDECAR_READY:
        return f"http://{host}"
    return f"https://{host}"
...
def apply_create_time_envs(sandbox, envs: Mapping[str, str]) -> bool:
    ...
    url = f"{_envd_init_base_url(host)}/init"
