...
with Sandbox.create(template=template_id) as sandbox:
    run(sandbox, "rustc --version")
    run(sandbox, "cargo --version")
    run(sandbox, "cd /workspace/hello-rust && cargo test")
    run(sandbox, "cd /workspace/hello-rust && cargo run --quiet")
    run(
        sandbox,
    ...
    run(
        ...
        "&& test \"$(cat /workspace/runtime-smoke/marker.txt)\" = rust-runtime-ok",
    )
