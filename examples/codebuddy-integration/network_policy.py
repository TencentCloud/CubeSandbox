...
"""Restrict CodeBuddy Code's egress to the LLM API and inject the key on the wire.
...
# CodeBuddy ships as a Node.js bundle that bundles its own CA store and ignores
...
def main() -> int:
    ...
    # Pick the same env-var CodeBuddy itself would use to read the key, so the
    ...
    # Let the Node-based CodeBuddy CLI trust the CubeEgress interception CA
    # (see note on DEFAULT_NODE_CA_BUNDLE); without this the vault path fails
    # TLS. Override via CODEBUDDY_NODE_EXTRA_CA_CERTS if the CA lives elsewhere.
    sandbox_env["NODE_EXTRA_CA_CERTS"] = os.environ.get(
        "CODEBUDDY_NODE_EXTRA_CA_CERTS", DEFAULT_NODE_CA_BUNDLE
    )

    print(f"Internet environment: {internet_environment()}")
