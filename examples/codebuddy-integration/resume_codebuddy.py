...
"""Demonstrate CodeBuddy Code session persistence across a CubeSandbox pause/resume.
...
CodeBuddy state directory survived the snapshot, and asks CodeBuddy to continue
...
def assert_state_survived(sandbox: Sandbox, workspace: str, state_dir: str) -> None:
    ...
    command = shell_join(
        ...
        # CodeBuddy session data lives under projects/<projectDir>/<sessionId>/.
        # Confirm at least one project directory exists, which is what ``-c``
        # and ``-r <id>`` consult when resuming.
        f"test -n \"$(ls -1 {quoted_state}/projects 2>/dev/null)\"",
...
def main() -> int:
    ...
    # credentials CodeBuddy caches under /workspace/.codebuddy, widening
    # exposure — for shared clusters prefer the default-deny + vault pattern in
    # network_policy.py.
    ...
    try:
        ...
        result_1 = run_turn(
            sandbox, args.workspace, turn_1_prompt, args.exec_timeout, codebuddy_env, model
        )
        ...
        assert_state_survived(sandbox, args.workspace, args.codebuddy_home)

        print("\n=== Turn 2: continue the work ===\n")
        # ``-c`` picks the most recent session CodeBuddy recorded under
        # $CODEBUDDY_CONFIG_DIR/projects/, which survived the snapshot.
        result_2 = run_turn(
            sandbox,
            args.workspace,
            turn_2_prompt,
