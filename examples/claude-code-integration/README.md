...
— Anthropic's official AI coding agent — inside a CubeSandbox MicroVM. The agent
edits files, runs commands, and reaches an LLM API entirely within an isolated,
reproducible sandbox.

This example ships:

- A `Dockerfile` that stacks Node.js + the Claude Code CLI on top of the CubeSandbox base
  image (envd already listens on `:49983`).
- `run_claude_code.py` — a headless one-shot run inside `/workspace`.
- `resume_claude_code.py` — pause/resume across two turns, proving `/workspace` and
  Claude Code's state directory survive the snapshot.
...
- `env_utils.py`, `_claude_common.py`, `.env.example`, `requirements.txt`.
...
├── env_utils.py               # .env loading, provider keys, Claude Code command builder
...
`ripgrep`, `jq`, and cleans apt/npm caches.
...
| `CLAUDE_PROVIDER` | Claude Code | `anthropic` (default), `openai`, `deepseek`, ... |
| `CLAUDE_MODEL` | Claude Code | Model id for the provider |
| `ANTHROPIC_API_KEY` | `envs=...` (direct) or CubeEgress inject (vault) | Provider key |
| `CLAUDE_LLM_HOST` | `network_policy.py` | LLM API host to allow; keep aligned with the provider |
...
All three scripts run Claude Code with `--print` and stream the output
...
Turn 1 asks Claude Code to write `/workspace/plan.md`, then `sandbox.pause()` snapshots
the VM. The script reconnects with `Sandbox.connect(sandbox_id)`, verifies
`/workspace/plan.md` and Claude Code's state directory (`/root/.claude`) survived, then
...
- Egress is default-deny — only the LLM host (`CLAUDE_LLM_HOST`) is reachable.
- CubeEgress attaches the provider key as an HTTP header on the wire
  ...
  placeholder.
- Because Claude Code runs on Node.js (which ignores the system CA store), the script
  sets `NODE_EXTRA_CA_CERTS` so Claude Code trusts the CubeEgress interception CA;
  without it the vault path fails with `Connection error`. Override the bundle
  path via `CLAUDE_NODE_EXTRA_CA_CERTS` if your image keeps the CA elsewhere.
...
| `Connection error` / TLS failure from Claude Code on the vault path | Claude Code runs on Node, which ignores the system CA store and won't trust the CubeEgress interception CA | The script sets `NODE_EXTRA_CA_CERTS` to the system bundle; override with `CLAUDE_NODE_EXTRA_CA_CERTS` if your CA lives elsewhere |
