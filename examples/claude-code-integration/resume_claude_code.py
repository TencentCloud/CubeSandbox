...
Turn 1 asks Claude Code to write ``/workspace/plan.md``, then the sandbox is paused.
Turn 2 reconnects to the same sandbox, verifies both ``/workspace`` and the Claude
Code state directory survived the snapshot, and asks Claude Code to continue.
...
DEFAULT_CLAUDE_STATE_DIR = "/root/.claude"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Working directory inside the sandbox. Defaults to CLAUDE_WORKSPACE.",
    )
    parser.add_argument(
        "--name",
        default="cube-claude-code-resume",
        help="Claude Code session name reused across both turns to keep the conversation.",
    )
    parser.add_argument(
        "--claude-state-dir",
        default=os.environ.get("CLAUDE_CODE_DIR", DEFAULT_CLAUDE_STATE_DIR),
    ...
    parser.add_argument(
        ...
        help="Sandbox lifetime in seconds. Defaults to CLAUDE_SANDBOX_TIMEOUT or 1800.",
    )
    parser.add_argument(
        "--exec-timeout",
        type=int,
        default=int_env("CLAUDE_EXEC_TIMEOUT", 900),
        help="Claude Code command timeout in seconds. Defaults to CLAUDE_EXEC_TIMEOUT or 900.",
    )
...
def run_turn(
    ...
    command = shell_join(
        ...
        claude_command(prompt, name=name),
    )
    return run_command(
        sandbox,
        command,
...
def assert_state_survived(sandbox: Sandbox, workspace: str, state_dir: str) -> None:
    ...
    ensure_success(result, "verify /workspace and Claude Code state survived pause/resume")
...
def main() -> int:
    ...
    require_provider_key(claude_provider())
    ...
    # SECURITY: like run_claude_code.py this demo keeps egress open and injects the
    # key per command. The pause() snapshot also captures the in-VM env and any
    # credentials Claude Code caches under /root/.claude, widening exposure — for
    ...
    try:
        ...
        result_1 = run_turn(
            sandbox, args.workspace, turn_1_prompt, args.name, args.exec_timeout, claude_env
        )
        ...
        assert_state_survived(sandbox, args.workspace, args.claude_state_dir)

        print("\n=== Turn 2: continue the work ===\n")
        result_2 = run_turn(
            sandbox, args.workspace, turn_2_prompt, args.name, args.exec_timeout, claude_env
        )
