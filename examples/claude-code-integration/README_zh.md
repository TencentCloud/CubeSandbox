...
（Anthropic 官方的 AI 编码 Agent）。Agent 在一个隔离、可复现的沙箱内编辑文件、执行命令并访问 LLM API。

本示例包含：

- 一个 `Dockerfile`：在 CubeSandbox 基础镜像上叠加 Node.js 与 Claude Code CLI（envd 已监听 `:49983`）。
- `run_claude_code.py`：在 `/workspace` 内的一次性无交互运行。
- `resume_claude_code.py`：跨两轮的 pause/resume，证明 `/workspace` 与 Claude Code 状态目录在快照后仍存在。
...
├── env_utils.py               # .env 加载、provider key、Claude Code 命令构造
├── _claude_common.py          # 共享的沙箱命令辅助（run/ensure/id）
...
| `CLAUDE_PROVIDER` | Claude Code | `anthropic`（默认）、`openai`、`deepseek` 等 |
| `CLAUDE_MODEL` | Claude Code | 对应 provider 的模型 id |
| `ANTHROPIC_API_KEY` | `envs=...`（直连）或 CubeEgress 注入（vault） | provider 密钥 |
| `CLAUDE_LLM_HOST` | `network_policy.py` | 放行的 LLM API host，需与 provider 对齐 |
...
三个脚本都以 `--print` 运行 Claude Code，输出直接流式打印到终端。
...
`Sandbox.connect(sandbox_id)` 恢复，校验 `/workspace/plan.md` 与 Claude Code 状态目录
（`/root/.claude`）仍在，再执行第二轮续写。沙箱生命周期用 `try/finally` 手动管理（不用
...
| vault 路径下 Claude Code 报 `Connection error` / TLS 失败 | Claude Code 基于 Node，忽略系统 CA 库，不信任 CubeEgress 拦截 CA | 脚本已把 `NODE_EXTRA_CA_CERTS` 指向系统 CA 包；若 CA 在别处，用 `CLAUDE_NODE_EXTRA_CA_CERTS` 覆盖 |
| 就绪探针超时 | 镜像缺少 envd | 确认 `FROM ghcr.io/tencentcloud/cubesandbox-base:...` |
| `pause()`/`connect()` 报错 | 平台版本过低不支持快照 | 升级 CubeSandbox 平台 |

## 参考

- 集成指南：[`docs/guide/integrations/claude-code.md`](../../docs/zh/guide/integrations/claude-code.md)
