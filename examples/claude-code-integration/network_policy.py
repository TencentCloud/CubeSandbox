...
# includes it; the CubeSandbox base image installs the CA into the system
# bundle below.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--host",
        default=None,
        help="LLM API host to allow. Defaults to CLAUDE_LLM_HOST or the provider default.",
    )
    parser.add_argument(
        "--workspace",
    ...
    parser.add_argument(
        "--name",
        default="cube-claude-code-egress",
        help="Claude Code session name for the egress demo run.",
    )
    parser.add_argument(
        "--sandbox-timeout",
        type=int,
...
def run_agent(sandbox: Sandbox, args: argparse.Namespace, envs: dict[str, str]):
    command = shell_join(
        ...
        claude_command(args.prompt, name=args.name),
    )
    return run_command(
        sandbox,
        command,
...
def main() -> int:
    ...
    host = args.host or claude_llm_host(provider)
    if not host:
        raise SystemExit(
            "Could not resolve the LLM host. Set CLAUDE_LLM_HOST in your .env or pass --host."
        )

    rules = build_rules(provider, host, secret)

    sandbox_env = build_claude_env(include_secrets=False)
    key_name = provider_key_name(provider)
    sandbox_env[key_name] = PLACEHOLDER_KEY
    # Let the Node-based Claude Code CLI trust the CubeEgress interception CA (see
    # note on DEFAULT_NODE_CA_BUNDLE); without this the vault path fails TLS.
    sandbox_env["NODE_EXTRA_CA_CERTS"] = os.environ.get(
        "CLAUDE_NODE_EXTRA_CA_CERTS", DEFAULT_NODE_CA_BUNDLE
    )
