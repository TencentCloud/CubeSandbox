...
在支持 KVM 的 Linux 环境（WSL / Linux 物理机 / 云上的裸金属均可）中，四步启动你的第一个沙箱：

1. **启动开发虚机**（已有 x86_64 裸金属 Linux 服务器可跳过）

克隆仓库并启动一台一次性的 OpenCloudOS 9 开发虚机，接下来的步骤都在虚机内以 root 身份执行：
...
# 新开一个终端：
cd CubeSandbox/dev-env && ./login.sh   # 以 root 登录虚机
```

> 详见[开发环境（QEMU 虚机）](./docs/zh/guide/dev-environment.md)。
...
> ```
>
> 详情见[快速开始 — 国内加速镜像](./docs/zh/guide/quickstart.md#第二步安装)。
