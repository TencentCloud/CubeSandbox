...
mod ivshmem {
    ...
    fn _test_live_migration_ivshmem(local: bool) {
        ...
        // Start the source VM
        let src_vm_path = clh_command("cube-hypervisor");
        let src_api_socket = temp_api_path(&guest.tmp_dir);
        let mut src_vm_cmd = GuestCommand::new_with_binary_path(&guest, &src_vm_path);
        src_vm_cmd
            .args([
    ...
    fn test_snapshot_restore_ivshmem() {
        ...
        let r = std::panic::catch_unwind(|| {
            ...
            crate::snapshot_and_check_events(&api_socket_source, &snapshot_dir, &event_path);
        });
        ...
        // The VM is automatically resumed after restore (vm_restore()
        // calls resume() internally), so the tail of the event log must
        // be exactly [restored, resuming, resumed]. This matches every
        // other snapshot/restore test in this file.
        let latest_events = [
            &MetaEvent {
                event: "restored".to_string(),
                device_id: None,
