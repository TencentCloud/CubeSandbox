...
decouple_unquiesced(void *cb_arg, int status)
{
	...
	if (status != 0) {
		SPDK_ERRLOG("Failed to unquiesce lvol '%s' after detaching export %s: %s\n",
			    d->lvol_name, d->uuid_str, spdk_strerror(-status));
...
decouple_cleared(void *cb_arg, int bserrno)
{
	...
	if (rc != 0) {
		SPDK_ERRLOG("Could not unquiesce lvol '%s' after detaching export %s: %s\n",
		 ...
		 * starting the unlock, so there is no safe public retry. Keeping d
		 * alive cannot recover that handle either: it only strands the RPC,
		 * ingest callback and lvstore. Finish with the error so management can
		 * unload or restart the lvstore. */
		d->bdev_quiesced = false;
		decouple_after_unquiesce(d);
	}
}
