...
# Merge the appended parameters into the existing cmdline. Remove existing
# values for each key once, then append every configured value for that key.
# This keeps the merge idempotent while allowing multi-value kernel parameters
# such as "console=ttyS0,115200 console=tty0".
merge_cmdline() (
    # Split the whitespace-delimited arguments without expanding glob characters.
    # The subshell keeps this option and IFS local to this invocation.
    set -f
    local IFS=$' \t\n'
    local merged_cmdline="$1" append="$2"
    local replaced_keys=" " param key
    for param in $append; do
        key="${param%%=*}"

        if [[ "$replaced_keys" != *" $key "* ]]; then
            # Drop existing parameters with the same key, in either key or
            # key=value form. Do this only for the first configured occurrence so
            # later values with the same key are preserved.
    ...
    printf '%s\n' "$merged_cmdline" | tr -s ' ' | sed -E 's/^ +//; s/ +$//'
)

# Repeated keys are preserved in order; the two console entries are intentional.
default_grub_cmdline() {
...
main() {
    ...
    local GRUB_CMDLINE_LINUX_APPEND
    GRUB_CMDLINE_LINUX_APPEND="$(default_grub_cmdline)"
    ...
    local existing_cmdline="" merged_cmdline
    ...
    merged_cmdline="$(merge_cmdline "$existing_cmdline" "$GRUB_CMDLINE_LINUX_APPEND")"
...
# Run when executed as a file or piped to bash, but not when sourced.
