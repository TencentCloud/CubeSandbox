...
The Digital Assistant (AgentHub) persists assistant instances, snapshots, templates, and operation history through the CubeOps service (CubeAPI is stateless after the CubeAPI/CubeOps decoupling in PR #984). In one-click deployments, `DATABASE_URL` is generated automatically from `CUBE_SANDBOX_MYSQL_HOST`, `CUBE_SANDBOX_MYSQL_PORT`, `CUBE_SANDBOX_MYSQL_USER`, `CUBE_SANDBOX_MYSQL_PASSWORD`, and `CUBE_SANDBOX_MYSQL_DB` when it is not set explicitly, and is consumed by CubeOps:
...
- writes `DATABASE_URL` (CubeOps) and `CUBE_PROXY_REDIS_*` (cube proxy) to `.one-click.env` so every service consumes the external endpoint;
