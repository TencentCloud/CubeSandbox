...
The Digital Assistant (AgentHub) uses MySQL through CubeAPI to persist assistant instances, snapshots, templates, and operation history. `install.sh` assembles `DATABASE_URL` from `CUBE_SANDBOX_MYSQL_HOST`, `CUBE_SANDBOX_MYSQL_PORT`, `CUBE_SANDBOX_MYSQL_USER`, `CUBE_SANDBOX_MYSQL_PASSWORD`, and `CUBE_SANDBOX_MYSQL_DB`, so you normally leave it unset. An explicit `DATABASE_URL` is honored **only when MySQL is external** (see the `CUBE_SANDBOX_MYSQL_HOST` / `_MANAGED` notes above); in the default bundled mode it is **ignored** and always reassembled from `CUBE_SANDBOX_MYSQL_*`, because the bundled container is created with those credentials.

On `--mode=upgrade` in external mode, an explicit `DATABASE_URL` in your `.env` wins — with one exception that depends on where MySQL now resolves:

- **MySQL resolved at a remote host** (`CUBE_SANDBOX_MYSQL_HOST` is off-box): a `DATABASE_URL` whose own host is loopback (`127.0.0.0/8`/`localhost`/`::1`) is **always ignored** and reassembled from `CUBE_SANDBOX_MYSQL_*`, regardless of its credentials or query string — a loopback DSN cannot reach the remote server, so it can only be a stale leftover. To keep a custom DSN here, point it at the same remote host.
- **MySQL resolved on the local host** (loopback host + `*_MANAGED=0`): only a DSN matching the **pre-unification bundled default** (`mysql://cube:cube_pass@127.0.0.1:3306/cube_mvp` — loopback host, default `cube`/`cube_pass`/`cube_mvp` credentials, no query string) is ignored and reassembled, so a deployment that customised only `CUBE_SANDBOX_MYSQL_PASSWORD` is not silently pointed at the wrong credentials. Give the DSN a non-default credential (or a query string / non-`mysql://` scheme) to have it treated as a deliberate override.

If your `.env` has no `DATABASE_URL` but the previous install persisted a **custom** DSN into `.one-click.env` (one carrying query parameters such as `?parseTime=true&charset=utf8mb4`, or a non-`mysql://` scheme), that custom value is **preserved** across the upgrade (a warning is printed) rather than being rebuilt from `CUBE_SANDBOX_MYSQL_*` — which would drop the extra parameters. The same remote-host rule applies: if the persisted DSN's host is loopback but MySQL now resolves at a remote host, it is **discarded and rebuilt** (re-supply it in `.env` with the remote host to keep the query parameters). A plain auto-generated DSN (`mysql://user:pass@host:port/db` with no query/fragment) is still rebuilt from `CUBE_SANDBOX_MYSQL_*` so credential changes propagate. To change a custom DSN on upgrade, set it explicitly in `.env`.

```bash
# Optional; ignored in bundled mode, honored only for external MySQL.
...
MySQL/Redis are configured with a single variable set; the `*_HOST` value
decides the mode. By default it is `127.0.0.1` (the bundled local containers).
To point CubeSandbox at an existing MySQL/Redis server instead, set `*_HOST` to
its address in `.env` before running `install.sh` (see `env.example`):
...
> The legacy `CUBE_EXTERNAL_MYSQL_*` / `CUBE_EXTERNAL_REDIS_*` names are
> deprecated and being removed. On the **first** install/upgrade off a
> pre-unification release, any non-empty legacy variable is **authoritatively
> migrated** onto the matching `CUBE_SANDBOX_*` name (overwriting the target
> value), any non-empty legacy `*_HOST` forces `*_MANAGED=0`, and the legacy keys
> are then cleared from the runtime `.one-click.env` (a deprecation warning is
> printed). After that first migration the runtime env no longer carries any
> `CUBE_EXTERNAL_*`, so on later upgrades leftover `CUBE_EXTERNAL_*` lines you
> forgot to delete from your own `.env` are **ignored** (with a warning) rather
> than re-applied — they can no longer silently roll a migrated deployment back
> to the old endpoint. Either way, do not keep both variable sets: set
> `CUBE_SANDBOX_*_HOST` / `*_MANAGED` directly and delete the `CUBE_EXTERNAL_*`
> lines from your `.env`.

#### Connecting to your own MySQL/Redis on the local host

The `*_HOST` heuristic assumes a loopback address means "use the bundled
container". If you run your **own** MySQL/Redis on the local host (so it listens
on `127.0.0.1`), set `*_MANAGED=0` so cube connects to it instead of trying to
start its own container on the same loopback port (which would collide):
...
`*_MANAGED` defaults to `auto` (managed when the host is loopback, external
otherwise). `*_MANAGED=1` forces the bundled container and requires a loopback
host. For convenience it accepts booleans as well as `auto`: `1`/`true`/`yes`/`on`
mean managed and `0`/`false`/`no`/`off` mean external.

A "loopback" host is any address in the whole `127.0.0.0/8` range (e.g.
`127.0.0.2`), plus `localhost` and `::1` — not just `127.0.0.1`. So an off-box
`*_HOST` (anything outside `127.0.0.0/8`/`localhost`/`::1`) is treated as
external, while any `127.x.y.z` literal is treated as loopback (bundled unless
`*_MANAGED=0`). In bundled mode the container always publishes on `127.0.0.1`, so
a non-`127.0.0.1` loopback `*_HOST` only affects the mode classification — clients
(CubeAPI/CubeMaster) still connect via `127.0.0.1`.

> **Switching modes on `--mode=upgrade`:** `install.sh` freezes the resolved
> managed/external decision into `.one-click.env`, and the upgrade merge preserves
> it. To flip a dependency between bundled and external on upgrade, set
> `*_MANAGED` explicitly in your `.env` (bundled → external: set `*_MANAGED=0`;
> external → bundled: set `*_MANAGED=1`). Changing only `*_HOST` is rejected with
> a clear error (bundled → external) or silently kept external (external →
> bundled), because the previous install's frozen decision wins. When you set
> `*_MANAGED=1` the merge also resets a stale external `*_HOST` back to the
> bundled loopback default so the forced-bundled decision is self-consistent.
> `*_MANAGED=auto` does NOT flip external → bundled on its own: `auto` is
> host-driven and the upgrade preserves the external `*_HOST`, so the dependency
> stays external — use `*_MANAGED=1` to force the bundled container.

When a dependency is external — either an off-box `*_HOST` or a loopback host
with `*_MANAGED=0` — `install.sh`:
