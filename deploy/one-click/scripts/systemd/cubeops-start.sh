...
# Keep Web Terminal defaults explicit for upgrades from runtime env files that
# predate these keys. Operators can override every value in .one-click.env.
export CUBE_TERMINAL_ENABLED="${CUBE_TERMINAL_ENABLED:-true}"
export CUBE_TERMINAL_ALLOWED_ORIGINS="${CUBE_TERMINAL_ALLOWED_ORIGINS:-}"
export CUBE_TERMINAL_GRANT_TTL_SECONDS="${CUBE_TERMINAL_GRANT_TTL_SECONDS:-60}"
export CUBE_TERMINAL_HANDSHAKE_TIMEOUT_SECONDS="${CUBE_TERMINAL_HANDSHAKE_TIMEOUT_SECONDS:-10}"
export CUBE_TERMINAL_PING_INTERVAL_SECONDS="${CUBE_TERMINAL_PING_INTERVAL_SECONDS:-20}"
export CUBE_TERMINAL_PONG_TIMEOUT_SECONDS="${CUBE_TERMINAL_PONG_TIMEOUT_SECONDS:-10}"
export CUBE_TERMINAL_WRITE_DEADLINE_SECONDS="${CUBE_TERMINAL_WRITE_DEADLINE_SECONDS:-10}"
export CUBE_TERMINAL_IDLE_TIMEOUT_MINUTES="${CUBE_TERMINAL_IDLE_TIMEOUT_MINUTES:-30}"
export CUBE_TERMINAL_MAX_LIFETIME_HOURS="${CUBE_TERMINAL_MAX_LIFETIME_HOURS:-8}"
export CUBE_TERMINAL_RECONNECT_GRACE_SECONDS="${CUBE_TERMINAL_RECONNECT_GRACE_SECONDS:-30}"
export CUBE_TERMINAL_REPLAY_BUFFER_BYTES="${CUBE_TERMINAL_REPLAY_BUFFER_BYTES:-262144}"
export CUBE_TERMINAL_MAX_FRAME_BYTES="${CUBE_TERMINAL_MAX_FRAME_BYTES:-65536}"
export CUBE_TERMINAL_STDIN_QUEUE_FRAMES="${CUBE_TERMINAL_STDIN_QUEUE_FRAMES:-8}"
export CUBE_TERMINAL_STDOUT_PENDING_BYTES="${CUBE_TERMINAL_STDOUT_PENDING_BYTES:-262144}"
export CUBE_TERMINAL_MAX_SESSIONS_PER_USER="${CUBE_TERMINAL_MAX_SESSIONS_PER_USER:-5}"
export CUBE_TERMINAL_MAX_SESSIONS_PER_REPLICA="${CUBE_TERMINAL_MAX_SESSIONS_PER_REPLICA:-200}"
export CUBE_TERMINAL_DRAIN_TIMEOUT_SECONDS="${CUBE_TERMINAL_DRAIN_TIMEOUT_SECONDS:-30}"
