...
# to both registries under the same :v0.7.2-rc3 tag; either default resolves
# to whatever the operator most recently published.
CUBE_LCM_IMAGE_INT_DEFAULT="cube-sandbox-int.tencentcloudcr.com/cube-sandbox/cube-lifecycle-manager:v0.7.2-rc3"
CUBE_LCM_IMAGE_CN_DEFAULT="cube-sandbox-cn.tencentcloudcr.com/cube-sandbox/cube-lifecycle-manager:v0.7.2-rc3"
...
# `-` (not `:-`) so an explicitly empty password stays empty (managed Redis
# without AUTH); same rule as up-cube-proxy.sh.
...
# TLS is derived only from CUBE_EXTERNAL_REDIS_TLS (same endpoint as every other
# Redis client, so it cannot differ per component); default off.
CUBE_LCM_REDIS_TLS="$(normalize_redis_tls "${CUBE_EXTERNAL_REDIS_TLS:-0}" "CUBE_EXTERNAL_REDIS_TLS")"
...
render_template_atomic \
  ...
  -e "s#__CUBE_LCM_REDIS_TLS__#$(escape_sed "${CUBE_LCM_REDIS_TLS}" '#')#g" \
