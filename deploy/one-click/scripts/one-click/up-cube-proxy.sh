...
# registries under the same :v0.7.2-rc3 tag; either default resolves
...
CUBE_PROXY_IMAGE_INT_DEFAULT="cube-sandbox-int.tencentcloudcr.com/cube-sandbox/cube-proxy:v0.7.2-rc3"
CUBE_PROXY_IMAGE_CN_DEFAULT="cube-sandbox-cn.tencentcloudcr.com/cube-sandbox/cube-proxy:v0.7.2-rc3"
...
# empty. Managed Redis without AUTH rejects a single-argument AUTH outright, so
# falling back to the bundled password would take every connection down.
CUBE_PROXY_REDIS_PASSWORD="${CUBE_PROXY_REDIS_PASSWORD-${CUBE_SANDBOX_REDIS_PASSWORD-ceuhvu123}}"
# "1" enables TLS on the Lua Redis client (managed Redis with in-transit
# encryption); default off keeps bundled Redis on plaintext. Derived only from
# CUBE_EXTERNAL_REDIS_TLS, like the DB from CUBE_EXTERNAL_REDIS_DB: every client
# shares one endpoint, so no client can be switched on its own.
CUBE_PROXY_REDIS_SSL="$(normalize_redis_tls "${CUBE_EXTERNAL_REDIS_TLS:-0}" "CUBE_EXTERNAL_REDIS_TLS")"
...
CUBE_PROXY_REGISTRY_REDIS_SSL="${CUBE_PROXY_REDIS_SSL}"
...
render_template_atomic \
  ...
  -e "s/__CUBE_PROXY_REDIS_SSL__/$(escape_sed "${CUBE_PROXY_REDIS_SSL}" '/')/g" \
...
render_template_atomic \
  ...
  -e "s#__CUBE_PROXY_REGISTRY_REDIS_SSL__#$(escape_sed "${CUBE_PROXY_REGISTRY_REDIS_SSL}" '#')#g"
