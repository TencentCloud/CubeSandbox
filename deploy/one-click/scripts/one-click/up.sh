...
if [[ -n "${CUBEMASTER_ROOTFS_ARTIFACT_STORE_DIR_CONFIGURED}" ]]; then
  ...
  append_env_export CUBEMASTER_ARTIFACT_STORE_EXPORT CUBEMASTER_ROOTFS_ARTIFACT_STORE_DIR "${CUBEMASTER_ROOTFS_ARTIFACT_STORE_DIR}"
...
if [[ -n "${CUBE_MASTER_ADDR:-}" ]]; then
  append_env_export CUBE_API_OPTIONAL_EXPORTS CUBE_MASTER_ADDR "${CUBE_MASTER_ADDR}"
fi
...
if [[ -n "${DATABASE_URL:-}" ]]; then
  ...
  append_env_export CUBE_API_OPTIONAL_EXPORTS DATABASE_URL "mysql://${mysql_user}:${mysql_password}@${mysql_host}:${mysql_port}/${mysql_db}"
fi
if [[ -n "${CUBE_SANDBOX_NODE_IP:-}" ]]; then
  append_env_export CUBELET_OPTIONAL_EXPORTS CUBE_SANDBOX_NODE_IP "${CUBE_SANDBOX_NODE_IP}"
fi
