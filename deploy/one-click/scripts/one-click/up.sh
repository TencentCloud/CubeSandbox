...
CUBEMASTER_OPTIONAL_EXPORTS=""
if [[ -n "${CUBEMASTER_ROOTFS_ARTIFACT_STORE_DIR_CONFIGURED}" ]]; then
  mkdir -p "${CUBEMASTER_ROOTFS_ARTIFACT_STORE_DIR}"
  CUBEMASTER_OPTIONAL_EXPORTS+="export CUBEMASTER_ROOTFS_ARTIFACT_STORE_DIR=\"${CUBEMASTER_ROOTFS_ARTIFACT_STORE_DIR}\"; "
...
start_with_pidfile \
  "cubemaster" \
  "export CUBE_MASTER_CONFIG_PATH=\"${CUBEMASTER_CFG}\"; ${CUBEMASTER_OPTIONAL_EXPORTS}\"${CUBEMASTER_BIN}\""
