...
# Callers must define die(); log() is optional.
#
# Toggle: CUBE_CUBEBOX_OS_IMAGE_ON_DATA=1 enables the softlink (default off).
# Keep the historical toolbox path unless the operator opts in.

if [[ "${ONE_CLICK_CUBEBOX_OS_IMAGE_LIB_LOADED:-0}" == "1" ]]; then
  return 0
fi
ONE_CLICK_CUBEBOX_OS_IMAGE_LIB_LOADED=1
...
# Default data-disk location for large template rootfs artifacts.
CUBEBOX_OS_IMAGE_DATA_DIR_DEFAULT="/data/cubebox_os_image"

cubebox_os_image_on_data_enabled() {
  case "${CUBE_CUBEBOX_OS_IMAGE_ON_DATA:-0}" in
...
_cubebox_os_image_trusted_dir() {
  ...
  case "${resolved_target}" in
    "${resolved_parent}"/*)
      ...
      # Only prior cubebox_os_image* dirs are migratable — never /data/cubelet etc.
      case "$(basename "${resolved_target}")" in
        cubebox_os_image|cubebox_os_image_*) ;;
        *) return 1 ;;
      esac
      return 0
      ;;
...
_migrate_cubebox_os_image_dir() {
  ...
  elif [[ -z "$(find "${data_path}" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null || true)" ]]; then
    # Empty dest: prefer mv only when rmdir succeeds. Mountpoints (k8s
    # DirectoryOrCreate hostPath) fail rmdir with EBUSY — fall through to cp
    # so mv cannot nest src inside the still-present destination.
    if rmdir "${data_path}" 2>/dev/null; then
      mv "${src}" "${data_path}" || return 1
    else
...
# Avoids filling the rootfs with large template images while cubelet keeps using
# the historical toolbox path (no cubelet config change required).

ensure_cubebox_os_image_on_data() {
  local toolbox_root="${1:-${TOOLBOX_ROOT:-${CUBE_SANDBOX_INSTALL_ROOT:-/usr/local/services/cubetoolbox}}}"
  ...
  if [[ "${data_path}" == "${link_path}" ]]; then
    if type log >/dev/null 2>&1; then
      log "ERROR: cubebox_os_image data dir must differ from toolbox link path: ${data_path}"
    fi
    return 1
  fi
  ...
  if [[ -L "${link_path}" ]]; then
    local current resolved_current resolved_data
    current="$(readlink "${link_path}")"
    resolved_current="$(_cubebox_os_image_resolve "${current}" 2>/dev/null || true)"
    resolved_data="$(_cubebox_os_image_resolve "${data_path}" 2>/dev/null || true)"
    if [[ "${current}" == "${data_path}" || ( -n "${resolved_current}" && -n "${resolved_data}" && "${resolved_current}" == "${resolved_data}" ) ]]; then
      mkdir -p "${data_path}"
      return 0
    fi
    # Keep the existing link until migrate succeeds; ln -sfn below replaces it.
    if _cubebox_os_image_trusted_dir "${current}" "${data_path}"; then
      ...
      if [[ -z "${resolved_current}" || -z "${resolved_data}" || "${resolved_current}" != "${resolved_data}" ]]; then
        if ! _migrate_cubebox_os_image_dir "${current}" "${data_path}"; then
          if type log >/dev/null 2>&1; then
            log "ERROR: failed to migrate cubebox_os_image symlink target ${current}; leaving prior link in place"
          fi
          return 1
        fi
        if [[ -d "${current}" && "${current}" != "${data_path}" ]]; then
          if _cubebox_os_image_is_mountpoint "${current}"; then
            if type log >/dev/null 2>&1; then
              log "WARN: leaving previous cubebox_os_image data dir (mountpoint): ${current}"
            fi
          else
            rm -rf "${current}"
          fi
