...
_is_managed_cubebox_os_image_symlink() {
  ...
  # Exact string match (common case).
  [[ "${current}" == "${expected}" ]] && return 0
  # Equivalent resolved paths (trailing slash / ..), or a prior managed target
  # under dirname(expected) so relocating CUBEBOX_OS_IMAGE_DATA_DIR does not
  # abort wipe before ensure can retarget.
  local resolved_current resolved_expected
  resolved_current="$(_cubebox_os_image_resolve "${current}" 2>/dev/null)" || return 1
  resolved_expected="$(_cubebox_os_image_resolve "${expected}" 2>/dev/null)" || return 1
  [[ -n "${resolved_current}" && -n "${resolved_expected}" ]] || return 1
  [[ "${resolved_current}" == "${resolved_expected}" ]] && return 0
  _cubebox_os_image_trusted_dir "${current}" "${expected}"
}
