...
test_assert_safe_install_prefix() {
  ...
  # Prior managed target under the same data parent is still accepted (relocation).
  local reloc_prefix="${TMP_DIR}/reloc-os-image"
  local reloc_root="${TMP_DIR}/reloc-data"
  local reloc_old="${reloc_root}/cubebox_os_image_old"
  local reloc_new="${reloc_root}/cubebox_os_image"
  mkdir -p "${reloc_prefix}" "${reloc_old}" "${reloc_new}"
  : > "${reloc_prefix}/.one-click.env"
  ln -sfn "${reloc_old}" "${reloc_prefix}/cubebox_os_image"
  (
    CUBEBOX_OS_IMAGE_DATA_DIR="${reloc_new}"
    assert_safe_install_prefix "${reloc_prefix}"
  ) >/dev/null 2>&1 \
    || fail "assert_safe_install_prefix should accept prior managed cubebox_os_image under same data parent"
...
test_ensure_cubebox_os_image_on_data() {
  ...
  [[ ! -e "${old_data}" ]] || fail "retarget should remove previous trusted data dir after success"

  # Failed retarget migrate must keep the prior symlink.
  local toolbox_rf="${TMP_DIR}/ensure-toolbox-retarget-fail"
  local data_root_rf="${TMP_DIR}/ensure-data-retarget-fail"
  local old_rf="${data_root_rf}/cubebox_os_image_old"
  local new_rf="${data_root_rf}/cubebox_os_image"
  local bin_rf="${TMP_DIR}/bin-stub-retarget-fail"
  mkdir -p "${toolbox_rf}" "${old_rf}" "${new_rf}" "${bin_rf}"
  : > "${old_rf}/cached.ext4"
  : > "${new_rf}/existing.txt"
  ln -s "${old_rf}" "${toolbox_rf}/cubebox_os_image"
  cat >"${bin_rf}/cp" <<'EOF'
#!/bin/sh
...
EOF
  chmod +x "${bin_rf}/cp"
  if ( PATH="${bin_rf}:${PATH}" ensure_cubebox_os_image_on_data "${toolbox_rf}" "${new_rf}" ) >/dev/null 2>&1; then
    fail "retarget ensure should fail when merge-copy cannot write"
  fi
  [[ -L "${toolbox_rf}/cubebox_os_image" ]] || fail "failed retarget must keep prior symlink"
  [[ "$(readlink "${toolbox_rf}/cubebox_os_image")" == "${old_rf}" ]] \
    || fail "failed retarget must keep prior symlink target"

  # Empty pre-created dest prefers mv (DirectoryOrCreate style).
  local toolbox_empty="${TMP_DIR}/ensure-toolbox-empty"
  local data_empty="${TMP_DIR}/ensure-data-empty/cubebox_os_image"
  mkdir -p "${toolbox_empty}/cubebox_os_image" "${data_empty}"
  : > "${toolbox_empty}/cubebox_os_image/via-mv.ext4"
  ensure_cubebox_os_image_on_data "${toolbox_empty}" "${data_empty}"
  [[ -L "${toolbox_empty}/cubebox_os_image" ]] || fail "empty-dest migrate should leave a symlink"
  [[ -f "${data_empty}/via-mv.ext4" ]] || fail "empty-dest migrate should mv content into data dir"

  unset CUBE_CUBEBOX_OS_IMAGE_ON_DATA
  ensure_cubebox_os_image_on_data "${toolbox4}" "${TMP_DIR}/ensure-data4/cubebox_os_image"
  [[ ! -e "${toolbox4}/cubebox_os_image" ]] || fail "default (unset) toggle must not create cubebox_os_image"
  CUBE_CUBEBOX_OS_IMAGE_ON_DATA=0
  ensure_cubebox_os_image_on_data "${toolbox4}" "${TMP_DIR}/ensure-data4b/cubebox_os_image"
