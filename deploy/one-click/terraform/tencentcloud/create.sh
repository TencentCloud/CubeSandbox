...
setup_env() {
	...
	CUBE_IMAGE_TAG="${TENCENTCLOUD_CUBE_IMAGE_TAG:-v0.6.0-rc3}"
...
tcr_build_and_push() {
	...
	echo -e "  ${GREEN}✓ Image tag to build & push: ${CUBE_IMAGE_TAG:-v0.6.0-rc3}${NC}"
	echo ""
...
prompt_deployment_env() {
	...
	select_env TENCENTCLOUD_CUBE_IMAGE_TAG "Cube component image tag" "v0.6.0-rc3" "dev"
...
TENCENTCLOUD_CUBE_IMAGE_TAG='${TENCENTCLOUD_CUBE_IMAGE_TAG:-v0.6.0-rc3}'
...
write_resolved_tfvars_file() {
	...
	jq -n \
		...
		--arg image_tag "${TF_VAR_image_tag:-${CUBE_IMAGE_TAG:-${TENCENTCLOUD_CUBE_IMAGE_TAG:-v0.6.0-rc3}}}" \
