...
| Event                          | When `triggered` is true                                                                                                                         | Reaction? |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ | :-------: |
...
| `pull_request_review_comment`  | inline review-comment body contains `command` **and** commenter matches allow-lists                                                              |     ✅    |
| `pull_request_review`          | submitted review's body contains `command` **and** reviewer matches allow-lists                                                                  |     ❌    |

Reaction column:

- ✅ — the action applies `reaction` (default 👀) to the triggering entity
  so the user gets immediate visual feedback.
- ❌ — GitHub has no reactions API for this entity. The gate still
  works, but no visual acknowledgement is posted. If you want one, add a
  follow-up step that calls `rest.reactions.createForIssueComment` on
  the PR thread, or post an issue comment.
...
| `event_action` | Raw `payload.action` (e.g. `opened`, `assigned`, `created`, `submitted`). Use to distinguish event subtypes.     |
...
| `comment_id`   | Triggering entity ID (issue comment / PR review comment / PR review); empty for non-comment events               |
| `comment_body` | Triggering comment or review body                                                                                |
| `commenter`    | Triggering commenter or reviewer login                                                                           |
...
### 1. PR review triggered by `pull_request` OR `@codebuddy review` anywhere on the PR

Subscribes to every PR-scoped event a reviewer might type the command in:
top-level PR comments, inline diff comments, and submitted reviews.
...
on:
  pull_request:
    types: [opened]
  issue_comment:                  # top-level PR / Issue comments
    types: [created]
  pull_request_review_comment:    # inline diff comments
    types: [created]
  pull_request_review:            # body of a submitted review
    types: [submitted]

jobs:
  review:
    runs-on: ubuntu-latest
    # Cheap gate: for issue_comment events require the comment to be on
    # a PR. Other event types are PR-scoped by definition.
