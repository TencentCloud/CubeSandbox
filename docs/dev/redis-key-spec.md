...
**`sandbox:lifecycle:events`** (sandbox lifecycle stream, cross-service contract)

| field | Meaning |
| --- | --- |
| `event_id` | Stable UUID generated by CubeMaster for cross-system idempotency |
| `op` | Internal operation: `create`, `delete`, `update`, or `state` |
| `sandbox_id` | Sandbox ID |
| `ts` | Event timestamp in Unix milliseconds |
| `payload` | Optional JSON metadata or state snapshot |

The Redis Stream entry ID is an internal cursor and ordering key. External
consumers must use `event_id` as the event identity.
