...
- `failure_policy`: sidecar failure handling.
  ...
  - **`fail_closed`**: `Select` returns a typed `FailClosedError`; `runScoreFilter` aborts the Score phase so create fails closed. API clients see `ErrorCode_SelectNodesFailed` with a sanitized category message (not `ErrorCode_Unknown`). Applies to sidecar/transport failures **and** Select-time validation (empty/invalid endpoint, bad timeout/weight); `plugin_conf` absent stays fail-open because no policy field is readable. **Highest blast radius:** an empty endpoint with the default weight `1.0` (including a typo'd key like `endpont:` that yaml.v3 ignores) aborts every create until fixed — use `disable: true` or explicit `weight: 0` for staged rollouts. Caller cancel / parent deadline does not trip the circuit breaker.
