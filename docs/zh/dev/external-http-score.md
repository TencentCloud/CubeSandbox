...
- `failure_policy`：sidecar 失败处理。
  ...
  - **`fail_closed`**：`Select` 返回类型化 `FailClosedError`；`runScoreFilter` 中止 Score，create 失败关闭。API 客户端看到 `ErrorCode_SelectNodesFailed` 与脱敏类别消息（不是 `ErrorCode_Unknown`）。覆盖 sidecar/传输失败 **以及** Select 时校验（空/非法 endpoint、非法 timeout/weight）；`plugin_conf` 缺失仍 fail-open（读不到 policy）。**最高爆破半径：** 空 endpoint + 默认 weight `1.0`（含被 yaml.v3 忽略的错键如 `endpont:`）会中止每次 create，直到修好——灰度请用 `disable: true` 或显式 `weight: 0`。调用方 cancel / 父级 deadline 不触发熔断计数。
