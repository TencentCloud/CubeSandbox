...
可用 `--format json`、`--format markdown` 或 `--format both` 选择输出文件集合；默认是 `both`。`--out` 被当作 CLI 自己的目录：未选中的格式**仅在能识别为先前 schedulerbench 报告时**才会删除已有的 `report.json` / `report.md`（JSON 看 `run_id` 是否以 `scheduler-sim-` 开头，Markdown 看标题是否为 `# Scheduler Simulator Benchmark`）。同名但无关的文件会保留。因此 `--format json` 只会清掉工具自己写出的旧 `report.md`。若要同时保留两份文件，请换一个空目录。
