...
加载阶段失败关闭。空 Profile 下，base `enable_filters` / `enable_scorers` 中的
未知名同样会失败关闭（`validateListedFilterNames` /
`validateListedScorerPluginConfPresent`）；升级后不要再期望 typo 的选择器名被
静默丢弃或只在 `NewSelector` 告警并跳过。
