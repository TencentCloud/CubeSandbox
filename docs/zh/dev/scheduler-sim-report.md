...
>
> 历史备注（2026-09-17 范围裁剪）：`template_local_pressure` 已随预留子系统一并
> 从出厂 `template_reuse` Profile 移除，现行为 image_score 0.7 /
> real_time_weighted_average 0.3；本实验未去掉 0.2 权重重跑。仿真中该打分器
> 恒为 100（sim 引擎不驱动在途计数，见下文说明），故表中数字对现网 0.7/0.3
> 组合仍有代表性。
