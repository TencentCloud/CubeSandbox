...
| [崩溃恢复 Worker](https://github.com/tencentcloud/CubeSandbox/tree/master/examples/crash-recovery-worker) | 让长期运行的有状态 Worker 连续经历多轮 checkpoint、崩溃、rollback 和任务重放，并在每次转账后验证进程内存与可写工作区。 |
