...
| [Go 开发沙箱](https://github.com/tencentcloud/CubeSandbox/tree/master/examples/go-dev-sandbox) | 基于 `cubesandbox-base` 的开箱即用 Go 工具链模板，包含「构建 → 运行 → 测试」示例、用快照打断点崩溃后回滚续跑的长时任务示例，以及经共享 host mount 在并行沙箱中扇出交叉编译矩阵的示例。 |
