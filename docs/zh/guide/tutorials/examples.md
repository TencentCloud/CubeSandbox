...
| [Webhook 接收端](https://github.com/tencentcloud/CubeSandbox/tree/master/examples/webhook-receiver) | 无第三方依赖的 Python 接收端，支持 HMAC-SHA256 验签、签名测试发送和可选的企业微信群机器人转发。 |

::: tip
沙箱 SDK 示例共享相同的环境变量约定（`E2B_API_URL`、`E2B_API_KEY`、`CUBE_TEMPLATE_ID`）。Webhook 接收端等控制面集成示例会单独说明所需变量。请先参考[快速开始](../quickstart.md)指南搭建 Cube Sandbox 环境。
:::
