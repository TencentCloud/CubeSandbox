...
| 示例 | 说明 |
|------|------|
| [代码沙箱快速入门](https://github.com/tencentcloud/CubeSandbox/tree/master/examples/code-sandbox-quickstart) | 最基础的用法：创建沙箱、执行 Python 代码、运行 Shell 命令、管理网络策略、宿主机 argv 工具白名单演示等，全部通过 E2B SDK 完成。 |
| [Agent 工具白名单（BYOI）](https://github.com/tencentcloud/CubeSandbox/tree/master/examples/agent-tool-allowlist-sandbox) | 可构建的工具集画像模板 + 宿主机 argv 门控（配合 quickstart 脚本）；可叠断网；非 LLM Agent。 |
