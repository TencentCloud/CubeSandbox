...
| [Go 运行时模板](https://github.com/tencentcloud/CubeSandbox/tree/master/examples/go-runtime-template) | 模板制作示例：基于 `cubesandbox-base` 构建可用于 Cube 的 Go 运行时镜像，验证 `envd`，并在沙箱工作区执行 `go version`、`go test` 和 `go run`。 |
| [Rust 运行时模板](https://github.com/tencentcloud/CubeSandbox/tree/master/examples/rust-runtime-template) | 模板制作示例：基于 `cubesandbox-base` 构建可用于 Cube 的 Rust 运行时镜像，验证 `envd`，并在沙箱工作区执行 `rustc`、`cargo test` 和 `cargo run`。 |
