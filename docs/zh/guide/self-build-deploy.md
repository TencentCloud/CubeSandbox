...
默认情况下，安装脚本通过 Docker Compose 启动内置的 `mysql:8.0` 容器承载控制平面的元数据。如果你已有一套数据库服务，可以直接连接外部实例，安装脚本会自动改写 `conf.yaml`、屏蔽本地 MySQL 容器（`cube-sandbox-mysql.service` 被 mask），并把连接串写入运行时环境。

控制平面同时支持 **MySQL** 和 **PostgreSQL** 两种引擎，二者互斥——只能配置其中一个外部数据库，若检测到冲突变量安装脚本会直接报错退出。

#### 统一接口（`CUBE_EXTERNAL_DB_*`）

在 `.env` 中设置 `CUBE_EXTERNAL_DB_HOST` 即启用外部数据库模式，`CUBE_EXTERNAL_DB_DRIVER` 选择引擎：

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `CUBE_EXTERNAL_DB_HOST` | （空） | 外部数据库主机；设置后启用外部 DB 模式 |
| `CUBE_EXTERNAL_DB_DRIVER` | `mysql` | 引擎：`mysql` 或 `postgres` |
| `CUBE_EXTERNAL_DB_PORT` | `3306` / `5432` | 端口（不设时按引擎自动选择默认值） |
| `CUBE_EXTERNAL_DB_USER` | `cube` | 用户名 |
| `CUBE_EXTERNAL_DB_PASSWORD` | `cube_pass` | 密码（务必改为强密码） |
| `CUBE_EXTERNAL_DB_NAME` | 同 `CUBE_SANDBOX_MYSQL_DB`（`cube_mvp`） | 数据库名 |
| `CUBE_EXTERNAL_DB_SSLMODE` | `disable` | （仅 postgres）TLS 模式：`disable` / `require` / `verify-ca` / `verify-full` |

启用后，CubeMaster 和 CubeOps 首次启动时会通过 goose 在目标库上执行迁移，创建全部所需表。请确保：

- 目标数据库（`CUBE_EXTERNAL_DB_NAME`）已存在；
- 配置的用户对该库有建表权限；
- 若服务端启用了 TLS，把 `CUBE_EXTERNAL_DB_SSLMODE` 设为 `require` 或更严格的 `verify-full`。

PostgreSQL 示例 `.env` 片段：
...
#### 旧版 MySQL 别名（`CUBE_EXTERNAL_MYSQL_*`）

旧版 `CUBE_EXTERNAL_MYSQL_*` 变量保留向后兼容支持，启动时会自动映射到对应的 `CUBE_EXTERNAL_DB_*` 并将 `DRIVER` 设为 `mysql`：
...
新部署建议使用 `CUBE_EXTERNAL_DB_*`，已有 `.env` 文件中的 `CUBE_EXTERNAL_MYSQL_*` 无需修改，继续生效。
