...
- **集群一致性**：自动暂停由 `cube-lifecycle-manager`（CLM）服务统一协调；它消费 CubeMaster 通过 Redis stream 发布的生命周期事件，通过 Redis 注册表实时发现所有在线的 CubeProxy 副本并广播状态。多副本环境下用带 owner 后缀的 Redis state key 互斥锁（Lua compare-and-set）确保同一沙箱不会被并发暂停或恢复。
- **CLM 高可用（active-standby）**：Kubernetes 部署默认运行 2 个 CLM 副本，通过 Redis 租约（`cube:v1:shared:lock:lifecycle_manager:leader`，默认 TTL 15s）选主：只有 leader 运行有状态循环——事件消费、空闲清扫、last-active 轮询，以及滞留 stream 待处理条目的接管（`XAUTOCLAIM`）。所有副本的 `/readyz` 都返回 200：readiness 只表示"进程健康"，响应 JSON 中的 `role` 字段（`leader` / `standby` / `standalone`）仅用于可观测性，因此 Service 会把 `/internal/resume` 流量路由到任意 Ready 的 Pod。备副本可以真正服务 resume：registry 未命中时直接从权威的 meta Hash 应答；跨副本的 resume 所有权是原子的——通过 Redis Lua compare-and-set 在沙箱 state key 上以每进程随机 owner token 完成（过渡态取值形如 `resuming@<owner>`），两个副本同时收到同一 resume 时只会产生一次实际的 CubeMaster resume RPC。leader 崩溃后备副本最长一个 TTL 内接管：先从 meta Hash 引导 registry，再异步地向每个 CubeProxy 重放快照（每个 proxy 独立 goroutine，总计 5 分钟上限，不可达的 proxy 不会阻塞接管），然后用 `XAUTOCLAIM` 接管旧 leader 滞留的 stream 待处理条目（min-idle 与接管节拍均为 `CUBE_LCM_RECONCILE_INTERVAL`，默认 60s）。本版本有意不包含周期性对账循环——完整的 meta Hash/proxy 收敛被推迟到后续变更，故障切换与 resume 路径都不依赖它。one-click 单机部署仍是单副本（不启用选主，行为与旧版本一致）。将既有单副本 CLM 部署升级为 HA 时，先以 `replicas: 1` 发布新版本（确认 Pod Ready），再扩容到 2——不要从任何 `/readyz` 上备副本为 NotReady 的构建直接扩容，chart 默认的 `maxUnavailable: 0` 策略会因新备副本永远不可用而卡死滚动升级。
