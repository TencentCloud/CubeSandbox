...
在 CubeSandbox MicroVM 内运行 [Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview)
（Anthropic 官方的终端 AI 编码 Agent）。本文覆盖镜像构建、密钥注入、出网管控，以及基于快照的会话持久化，配套的可运行示例位于
...
| 组件 | 版本 |
|---|---|
| Claude Code | `@anthropic-ai/claude-code`（通过 npm 安装） |
...
| `CLAUDE_PROVIDER` / `CLAUDE_MODEL` | Claude Code CLI 参数 | 选择 provider 与模型 |
| `ANTHROPIC_API_KEY` | `envs=...`（直连）或 CubeEgress 注入（vault） | provider 密钥 |
| `ANTHROPIC_BASE_URL` | 传入 exec 环境 | Anthropic 兼容网关（如 DeepSeek） |
| `CLAUDE_LLM_HOST` | `network_policy.py` | 默认拒绝出网下放行的 LLM host |

### 4. 运行时配置与 API Key 注入

Claude Code 以无交互方式构造：`-p`（`--print`，处理完 prompt 即退出，不启动 TUI），加 `--permission-mode acceptEdits` 在隔离沙箱内跳过所有权限提示（沙箱无法触及宿主，故安全）。输出为纯文本。两种密钥流转方式共用同一个模板：

**直连方式** —— 逐命令传入密钥。SDK 的 `commands.run(envs=...)` 把环境放进 exec 信封，而非 VM 内的持久文件，因此密钥只在该命令执行期间存在：
...
示例脚本将 Claude Code 的输出直接流式打印到终端。
...
- `Sandbox.connect(sandbox_id)` 恢复时，`/workspace`、Claude Code 状态目录（`/root/.claude`）及其他文件都完好无损。
...
try:
    ...
    assert_state_survived(sandbox)       # /workspace + /root/.claude 仍在
...
- **Agent 状态目录。** `/root/.claude` 保存 Claude Code 的会话缓存。镜像里保持它为空，避免跨租户泄露会话；它在构建时创建但不写入任何凭证。
- **直连方式的密钥留存。** 直连方式（`envs=`）下密钥仅作用于该 exec 调用，但 Claude Code 可能把 provider 凭证缓存到其状态目录（`/root/.claude/`），会在 `pause()` / `resume()` 后仍留在盘上。对隔离要求高时优先用保险柜方式（`network_policy.py`），密钥完全不进入 VM。
- **CubeEgress CA（Node）。** 保险柜方式要求沙箱信任 CubeEgress 根 CA，基础镜像已把它装入系统 CA 包。
  ...
  （可用 `CLAUDE_NODE_EXTRA_CA_CERTS` 覆盖）——否则 vault 路径会以 `Connection error` 失败。
- **出网副作用。** 需要 `npm install` 或拉取 MCP 工具的任务，要放行相应 host 或预装进模板。
- **交互式 TTY 功能。** Claude Code TUI 在 E2B 协议下不可用。请用无交互 `-p --permission-mode acceptEdits`，多轮对话由宿主脚本驱动。
...
| vault 下 Claude Code 报 `Connection error` / TLS 失败 | Claude Code 的 Node 运行时忽略系统 CA 库，不信任 CubeEgress CA | 示例已设 `NODE_EXTRA_CA_CERTS`；若 CA 在别处用 `CLAUDE_NODE_EXTRA_CA_CERTS` 覆盖 |
