...
`configuredQos` 表示记录的 QoS 配置。`qosApplied` 表示 CubeMaster 保存的沙箱创建请求中是否包含至少一个网络或块设备 QoS annotation。这是创建时的配置记录，不能证明 limiter 已在 hypervisor 生效，也不能证明每项限制都已配置。如果只有 `configuredQos`，需要检查沙箱创建请求中是否缺少 QoS annotation，或相关记录是否不可用。
