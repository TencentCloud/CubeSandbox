...
| `quota_cpu` | `host.quota.mcpu_limit` 或宿主机推导值 | CPU 可调度容量（毫核）。Cubelet 在推导 quota 时已内置超卖因子，CubeMaster 直接使用上报值。 |
| `quota_mem_mb` | `host.quota.mem_limit` 或宿主机推导值 | 内存可调度容量（MB）。Cubelet 在推导 quota 时已内置超卖因子，CubeMaster 直接使用上报值。 |
...
- `host.quota.mcpu_limit`、`host.quota.mem_limit`、`host.quota.mvm_limit` 是否仍为默认推导值。超卖由 Cubelet 在推导 quota 时应用，而非 CubeMaster。
