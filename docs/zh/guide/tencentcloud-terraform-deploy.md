...
| `TENCENTCLOUD_BALLOON_FREE_PAGE_REPORTING` | 未设置 | 可选的 `on`/`off` 覆盖值，会传递给每台计算节点上的 CubeShim。首次部署时未设置会使用架构默认值（aarch64 关闭，其他架构开启）；重新运行部署器时未设置则不改变已安装策略。设置为 `default` 可删除已持久化的覆盖值。该设置仅影响新启动的 shim 进程和新生成的 VM 配置 |
