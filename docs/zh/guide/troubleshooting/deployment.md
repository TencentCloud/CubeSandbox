...
| E2B 兼容客户端出现 `dial tcp <proxy-ip>:443: connect: connection refused` | CubeProxy 监听在非默认 HTTPS 端口，但 CubeAPI 发布的沙箱域名没有带端口。E2B 兼容客户端会默认访问 `443`，导致上传文件、执行命令或启动 runner 的数据面请求失败。设置 `CUBE_API_SANDBOX_DOMAIN=<domain>:<https-port>` 并重启 `cube-api`。 | [HTTPS 证书与域名解析](../https-and-domain.md#非默认-cubeproxy-https-端口) |
