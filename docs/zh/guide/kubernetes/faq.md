...
节点怎么打标签见 [Helm 安装 · 打标签](./install.md#4-给节点打标签及角色污点)。

### 计算面 DaemonSet Ready 数不够

先看三条 Advanced DaemonSet 与原生 PVM DaemonSet：
...
也可一行执行（与 Chart `NOTES.txt` 一致；若本机 `kubectl` 能解析 CloneSet）：
...
正常链路（摘要）：ensure `pvm-not-ready` → 腾空依赖 Pod → invalidate ready 标记 → 换核 / reboot → 写 `pvm-host-ready` 并清闩 → bootstrap 写 `node-prep-ready` → Big Pod `wait-node-prep` Ready。完整说明见[架构说明 · PVM](./architecture.md#pvmcube-node-pvm)。
...
若只想滚动 Proxy 副本（CloneSet，不能用 `kubectl rollout restart`）：
...
**日常只改镜像 tag：默认不会。** Big Pod 用 `InPlaceIfPossible`，UID / IP / netns 可保持不变。操作步骤与红线见 [升级](./upgrade.md)。

会 recreate 的典型操作：增删 Big Pod 容器、改 volumeMount / securityContext / 容器名 / 直接改 env。

若要从旧 Chart（仍叫 `*-proxy-node`）升上来，Proxy 资源改名会短暂中断一次，见[升级 · 特殊场景](./upgrade.md#特殊场景)。
