...
为什么必须先装 OpenKruise？CubeSandbox 的控制面（CloneSet）和计算面主路径（Advanced DaemonSet）依赖它；若先打角色污点再装 OpenKruise，`kruise-controller-manager` 很容易一直 Pending。
...
| 项目 | 要求 |
| --- | --- |
| Kubernetes | **v1.27+**（自建 / k3s / TKE 等均可），并开启 `InPlacePodVerticalScaling` |
| 工具 | `kubectl`、Helm v3.10+ |
| OpenKruise | **1.9+，硬依赖**（下一步安装；需开启 `InPlaceWorkloadVerticalScaling`） |
| 存储 | 集群有可用 StorageClass（或改用 hostPath，见下文） |
| 镜像拉取 | 能拉取 Chart 默认镜像，或自备私有仓库 |

::: tip 为什么要 1.27+ 并开 FG？
日常镜像原地升级（`InPlaceIfPossible`）在更低版本也能用。但 Big Pod 槽位后续若要**原地改 resources（CPU/Memory）**以加组件，依赖 Kubernetes 的 `InPlacePodVerticalScaling`（1.27 Alpha，需显式打开；1.33+ 默认开启）以及 OpenKruise 的 `InPlaceWorkloadVerticalScaling`。因此安装基线按 **1.27+ + FG** 要求。
:::
...
并确认集群已开启 **`InPlacePodVerticalScaling`**（自建集群查 apiserver / kubelet 的 `--feature-gates`；托管集群查控制台或发行版文档。Kubernetes **1.33+** 默认开启）。
...
> **硬约束：** 先完成本节并确认 Ready，再打下一节的**角色污点**。
...
期望：`kruise-controller-manager` Ready ≥ 1，各节点上的 `kruise-daemon` Ready，且能发现 Advanced DaemonSet / CloneSet API。

---

## 4. 给节点打标签（及角色污点）

OpenKruise 已 Ready 后，再标记节点。

### 4.1 多节点（控制面 / 计算面分离，推荐）
...
### 4.3 启用 PVM 前的系统组件检查（需要换核时）

确认 CNI、kube-proxy、`kruise-daemon` 能容忍 `NoSchedule`（`operator: Exists`，或显式容忍 `cube.tencent.com/pvm-not-ready`），并在目标节点上仍为 Running。否则 PVM 门闩期间节点可能失联，无法清闩。
...
| 现象 | 怎么处理 |
| --- | --- |
| `kruise-controller-manager` Pending | 是否先打了 `control`/`compute` 污点？先去掉污点或给 manager 加容忍，装好 Kruise 再打污点 |
