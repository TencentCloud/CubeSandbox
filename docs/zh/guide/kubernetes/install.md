...
空闲页上报在 aarch64 上默认关闭，在其他架构上默认开启。如需覆盖该默认值，可在
`runtime-values.yaml` 中为新建沙箱配置：
...
该修改会改变 `cube-node` Pod 模板。应用到已有部署前，请遵循计算面升级流程。新值仅
影响 Pod 重建后新启动的 shim 进程和新生成的 VM 配置；运行中的沙箱及现有快照保存的
设备拓扑不会改变。在 aarch64 上，升级前创建的模板会在快照中保留已开启的上报配置，
必须 redo 模板后才会采用新的默认关闭行为。
