...
* **🌐 跨机暂停与恢复（Preview）**：配合 S3 快照后端，已暂停的沙箱可在集群内任意兼容节点恢复，也可从快照在任意节点创建新沙箱 —— 沙箱不再绑定单一节点，节点维护或故障不再困住你的工作负载。
...
* [跨机快照](./cross-node-snapshot.md) — S3 后端下在集群内任意节点恢复暂停沙箱或从快照创建沙箱。
