...
CubeOps 当前采用单管理员授权模型：所有通过认证的访问令牌都具有管理员权限，沙箱记录也没有可供 CubeOps 校验的所有者或 ACL。因此终端功能**不提供租户隔离**。不要把 CubeOps 暴露给互不信任的用户；在多租户控制面使用前，必须先增加沙箱所有权或 ACL 校验。

票据防重放记录和会话计数保存在单个 CubeOps 进程内。项目提供的部署默认只运行一个 CubeOps 副本；如果要水平扩容，必须先增加共享的重放与会话状态存储，才能把“票据只使用一次”和“最多 4 个会话”视为集群级限制。
:::
