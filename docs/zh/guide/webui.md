...
处于 `running` 状态的沙箱会在 **Sandboxes** 列表行操作区和沙箱详情页标题栏显示终端按钮。点击后即可在浏览器中打开容器内的交互式 `/bin/sh`，浏览器不会直接访问 CubeMaster 或 Cubelet。

- 沙箱必须处于 `running` 状态。暂停中、已暂停、已停止或失败的沙箱会禁用终端按钮；请先恢复沙箱。
- 如果沙箱包含多个容器，可通过终端工具栏中的选择器切换目标容器。切换容器会关闭当前 shell，并创建一个相互独立的新会话。
- 创建终端会话沿用 CubeAPI 现有的鉴权和授权规则。WebSocket URL 只包含短期、一次性的 session ID，不会暴露 API Key 或 WebUI session token。
- 默认空闲超时为 30 分钟，可通过 CubeAPI 的 `TERMINAL_IDLE_TIMEOUT_SECS` 调整。键盘输入、终端输出和窗口 resize 都会刷新空闲计时。
- shell 退出或网络异常断开后，可使用工具栏中的重新连接按钮。重新连接会创建新的 shell，不会恢复已断开 shell 的进程状态；后端会话的滚动内容也不会重新拉取。

终端支持 ANSI 颜色、光标应用、复制粘贴、滚动回溯、`Ctrl+C`、窗口自适应和全屏模式。浏览器粘贴可能需要剪贴板权限。目前启动命令固定为 `/bin/sh`，可用工具取决于所选容器镜像（最小化镜像可能不包含 `top`、`ping` 等命令）；暂不支持文件传输、会话持久化或重新附着到同一个 shell 进程。

::: warning 终端权限等同于容器内管理权限
获准创建终端会话的用户可以使用容器当前用户的权限执行命令。共享环境或可从外网访问的部署应开启 CubeAPI 鉴权，并在 CubeAPI 日志中审查 `terminal.*` 审计事件。
:::
