...
| [Scenario Demos](https://github.com/tencentcloud/CubeSandbox/tree/master/examples/sandbox-patterns) | Four CubeSandbox platform patterns — stateful workspace lifecycle, egress policy enforcement, checkpoint-driven iterative development (snapshot/clone/rollback), and multi-sandbox collaboration with role-based network isolation. Uses a Rust template as the workload vehicle. |
