...
| [Go Dev Sandbox](https://github.com/tencentcloud/CubeSandbox/tree/master/examples/go-dev-sandbox) | A ready-to-use Go toolchain template built on `cubesandbox-base`, with a `build → run → test` demo, a long-running job that checkpoints via snapshot and resumes after a crash by rolling back, and a cross-compile matrix fanned out across parallel sandboxes over shared host mounts. |
