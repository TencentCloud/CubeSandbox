...
`configuredQos` is the recorded QoS configuration. `qosApplied` indicates whether the sandbox create request persisted by CubeMaster contains at least one network or block I/O QoS annotation. This is a record of create-time configuration, not confirmation that a limiter took effect in the hypervisor or that every limit was configured. If only `configuredQos` is present, check whether the sandbox create request lacks QoS annotations or its record is unavailable.
