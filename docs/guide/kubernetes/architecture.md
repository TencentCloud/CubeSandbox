...
| --- | --- | --- | --- |
| Control plane | CubeMaster | OpenKruise CloneSet + Service + Secret + PVC/hostPath | Node registration, template/rootfs artifacts, embedded DB migration, scheduling/metadata |
| Control-plane API | CubeAPI | CloneSet + Service | External E2B-compatible HTTP API; reads/writes MySQL; talks to CubeMaster |
| Ops backend | CubeOps | CloneSet + Service | JWT ops API + WebUI SDK; listens on `0.0.0.0:3010`; reads/writes MySQL; talks to CubeMaster |
| Admin entry | WebUI | CloneSet + Service + ConfigMap | Static console; reverse-proxies `/opsapi/` and `/cubeapi/v1/` to CubeOps (depends on `cubeOps.enabled`) |
| Ops entry | cubemastercli | CloneSet | CLI for `kubectl exec`; injects this Release’s CubeMaster endpoint |
| Dependent storage | MySQL / Redis | Built-in StatefulSet or third-party | Business data / Proxy and lifecycle state |
| Compute · runtime | `cube-node` (Big Pod) | OpenKruise Advanced DaemonSet (InPlaceIfPossible) | `wait-node-prep` + cubelet / network-agent + optional egress; **no initContainers** |
| Compute · artifacts | `cube-node-installer` | Advanced DaemonSet (Standard) | Installs shim / kernel / guest into the host toolbox |
| Compute · node bootstrap | `cube-node-bootstrap` | Advanced DaemonSet (Standard) | `wait-pvm-host`, `cube-node-init`, writes `node-prep-ready` |
...
| Data-plane entry | CubeProxy + cluster DNS | CloneSet; optionally rewrites CoreDNS | HTTP/HTTPS sandbox entry; `*.domain` wildcard resolution |
| Lifecycle | cube-lifecycle-manager | CloneSet + ClusterIP | sandbox pause/resume; discovers Proxy replicas via Redis |
...
The three compute lines (Big Pod / installer / bootstrap) are OpenKruise Advanced DaemonSets: Big Pod uses `InPlaceIfPossible`; bootstrap/installer use `Standard`. **PVM is a native `apps/v1` DaemonSet** (does not depend on kruise-manager to create Pods). Stateless control plane (master/api/ops/webui/proxy/lifecycle/cubemastercli) uses CloneSet; MySQL/Redis continue to use native StatefulSets.

#### Big Pod: `cube-node`

- `hostNetwork: false` (Pod network); **always** Advanced DaemonSet + `InPlaceIfPossible` (hard OpenKruise dependency).
- **Zero initContainers**; the container set is the upgrade freeze surface (add/remove containers / change volumeMount / change env will recreate and break PodIP/netns).
...
| --- | --- | --- |
| `wait-node-prep` | `images.waitNodePrep` | Kruise priority 10 sidecar: read-only hostPath `node-prep-ready` self-describing fingerprint and continuous re-check |
| `network-agent` | `images.networkAgent` | Starts after self-stage; priority 0 |
| `cubelet` | `images.cubelet` | Starts after self-stage; priority 0 |
| `cube-slot-1`…`cube-slot-6` | `images.pause` | Frozen placeholder slots; same mounts/privileges as cubelet; later InPlace image/resource swaps only |
| `cube-egress` / `cube-egress-net` | matching images | Optional; transparent egress / TPROXY |

Placeholder slot service names are written in Pod annotations: `cube.tencent.com/slot-N` (values: `cubeNode.placeholderSlots.services`). An empty string means unassigned; annotations can later be changed in place (container env `CUBE_SLOT_SERVICE` refreshes via fieldRef). Per-slot image/resource overrides use `cubeNode.placeholderSlots.overrides."<N>"`.

::: tip Slot InPlace and cluster version
Later, if you need to **add components in place to placeholder slots and change resources**, you need Kubernetes **1.27+** with `InPlacePodVerticalScaling` enabled, plus OpenKruise `InPlaceWorkloadVerticalScaling`. See [Helm Install · Prerequisites](./install.md#1-prerequisites).
:::

**Container names / volumeMount / securityContext / imagePullPolicy must not change** (would recreate).
...
- Native `apps/v1` DaemonSet (not ADS); created only when `bootstrap.pvmHostKernel.enabled=true`; scheduled only to `placement.pvm`.
...
- Only the PVM DaemonSet tolerates the temporary gate. CNI, kube-proxy, and **kruise-daemon** must tolerate the gate with `Exists` or an explicit key; `kruise-controller-manager` Ready is a hard gate. PVM stays on Pod network.
- For PVM image upgrades **only bump `images.pvmHostBootstrap`**; do not recreate the Big Pod.

Why four pieces: keep the Big Pod InPlace-friendly; separate artifact install from rebootable PVM bootstrap; non-PVM compute nodes do not pull the large PVM image.
