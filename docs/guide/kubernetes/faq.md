...
How to label nodes: [Helm Install · Label nodes](./install.md#4-label-nodes-and-role-taints).

### Compute DaemonSet Ready count is short

First check the three Advanced DaemonSets and the native PVM DaemonSet:
...
Or one-liner (same as Chart `NOTES.txt`; if local `kubectl` can resolve CloneSet):
...
Normal path (summary): ensure `pvm-not-ready` → drain dependent Pods → invalidate ready markers → kernel swap / reboot → write `pvm-host-ready` and clear the gate → bootstrap writes `node-prep-ready` → Big Pod `wait-node-prep` Ready. Full details: [Architecture · PVM](./architecture.md#pvmcube-node-pvm).
...
If you only want to roll Proxy replicas (CloneSet; `kubectl rollout restart` does not work):

```bash
kubectl -n cube-system delete pod -l app.kubernetes.io/component=cube-proxy
```
...
**Day-to-day tag-only image bumps: by default no.** The Big Pod uses `InPlaceIfPossible`; UID / IP / netns can stay unchanged. Steps and red lines: [Upgrade](./upgrade.md).

Typical recreate triggers: add/remove Big Pod containers, change volumeMount / securityContext / container name / env directly.

If upgrading from an older Chart that still used `*-proxy-node`, renaming Proxy resources causes one brief interruption; see [Upgrade · Special cases](./upgrade.md#special-cases).
