...
| E2B-compatible clients fail with `dial tcp <proxy-ip>:443: connect: connection refused` | CubeProxy is listening on a non-default HTTPS port, but CubeAPI is publishing a sandbox domain without the port. E2B-compatible clients then default to `443` for file upload, command execution, and runner startup requests. Set `CUBE_API_SANDBOX_DOMAIN=<domain>:<https-port>` and restart `cube-api`. | [HTTPS & Domain Resolution](../https-and-domain.md#non-default-cubeproxy-https-ports) |
