...
- **Cluster coordination**: auto-pause is driven by the `cube-lifecycle-manager` (CLM) service that runs on the control node. It consumes lifecycle events CubeMaster publishes via Redis stream, discovers every live CubeProxy replica through a Redis-backed registration table, and broadcasts state to each of them. Cross-replica races are resolved by owner-tagged Redis state-key locks (a Lua compare-and-set) so the same sandbox is never paused or resumed twice concurrently.
- **CLM high availability (active-standby)**: Kubernetes deployments run 2 CLM replicas by default, electing a leader via a Redis lease (`cube:v1:shared:lock:lifecycle_manager:leader`, default TTL 15s). Only the leader runs the stateful loops — event consumer, idle sweeper, last-active poller, and stale-pending-entry takeover (`XAUTOCLAIM`). Every replica reports 200 on `/readyz`: readiness only means "process healthy", and the JSON body's `role` field (`leader` / `standby` / `standalone`) is purely observability, so the Service routes `/internal/resume` traffic to any Ready pod. A standby genuinely serves resume — on a registry miss it answers from the authoritative meta Hash — and cross-replica resume ownership is atomic: a Redis Lua compare-and-set on the sandbox state key with a per-process random owner token (transition values look like `resuming@<owner>`) means two replicas receiving the same resume concurrently issue exactly one actual CubeMaster resume RPC. If the leader crashes, a standby takes over within at most one TTL: it bootstraps its registry from the meta Hash, replays the snapshot to every CubeProxy asynchronously (per-proxy goroutines, 5-minute total bound, so an unreachable proxy never blocks the takeover), then claims the dead consumer's pending stream entries via `XAUTOCLAIM` (min-idle and takeover cadence: `CUBE_LCM_RECONCILE_INTERVAL`, default 60s). This release deliberately ships no periodic reconciliation loop — full meta-Hash/proxy convergence is deferred to a follow-up change, and neither failover nor resume depends on it. One-click single-node deployments stay single-replica (leader election disabled, behavior identical to previous releases). When moving an existing single-replica CLM deployment to HA, roll out the new version with `replicas: 1` first (verify it becomes Ready), then scale to 2 — never scale out from a build where standbys report NotReady on `/readyz`, since the chart's default `maxUnavailable: 0` strategy would then deadlock the rollout.
