...
- **CLM high availability (active-standby)**: Kubernetes deployments run 2 CLM replicas by default, electing a leader via a Redis lease (`cube:v1:shared:lock:lifecycle_manager:leader`, default TTL 15s). Only the leader runs the event-consumer, idle-sweeper, and reconciliation loops; standby replicas report 503 on `/readyz`, so the Service routes `/internal/resume` traffic to the leader only. If the leader crashes, a standby takes over within at most one TTL: it first bootstraps fully from the meta Hash, then adopts the dead leader's stale stream pending entries via `XAUTOCLAIM`, and a periodic reconciliation loop (default 60s) converges residual drift. During the switch window itself (old leader down → new leader ready) the Service has no ready endpoints, so resume requests fail with 503 + `Retry-After` and clients retry; the meta-Hash fallback (registry miss → look the sandbox up directly in Redis) exists for the freshly-promoted leader whose bootstrap hasn't landed yet, not for bypassing the readiness gate. One-click single-node deployments stay single-replica (leader election disabled, behavior identical to previous releases).
