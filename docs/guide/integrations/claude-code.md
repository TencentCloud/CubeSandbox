...
Run [Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview)
— Anthropic's official terminal-native AI coding agent — inside CubeSandbox
MicroVMs. This guide covers image build, key injection, egress control, and
snapshot-based session persistence, and pairs with the runnable
...
| Component | Version |
|---|---|
| Claude Code | `@anthropic-ai/claude-code` (installed via npm) |
...
| `CLAUDE_PROVIDER` / `CLAUDE_MODEL` | Claude Code CLI flags | Provider and model selection |
...
| `CLAUDE_LLM_HOST` | `network_policy.py` | LLM host allowed under default-deny egress |

### 4. Runtime Configuration and API Key Injection

Claude Code is invoked headlessly with `--print` (process the prompt and exit, no
TUI), and ``--permission-mode acceptEdits`` to skip permission prompts inside the
isolated sandbox (safe because the agent cannot access the host). Output is
plain text written to stdout. Two key-flow flavors share the same template:

**Direct flavor** — forward the key per command. The SDK's `commands.run(envs=...)`
...
result = sandbox.commands.run(
    "# sandbox: auto-accept file edits\n"
...
The example scripts stream Claude Code's output directly to the terminal.
...
- `Sandbox.connect(sandbox_id)` resumes with `/workspace`, Claude Code's state
  directory (`/root/.claude`), and every other file intact.
...
try:
    ...
    assert_state_survived(sandbox)       # /workspace + /root/.claude intact
...
- **Agent state directory.** `/root/.claude` holds Claude Code's session cache.
...
- **Direct-flavor key persistence.** With the direct flavor (`envs=`) the key is
  scoped to the exec call, but Claude Code may cache provider credentials under
  its state dir (`/root/.claude/`), which survives `pause()` / `resume()`. For
...
- **CubeEgress CA (Node).** For the vault flavor the sandbox must trust the
  ...
  `CLAUDE_NODE_EXTRA_CA_CERTS`) — without it the vault path fails with
  `Connection error`.
- **Egress side-effects.** Tasks that `npm install` or fetch MCP tools need those
  hosts allowed or preinstalled into the template.
- **Interactive TTY features.** The Claude Code TUI is not available over the E2B
  protocol. Use headless `-p --permission-mode acceptEdits` and drive multi-turn
  conversations from
  the host script.
...
| `Connection error` / TLS failure from Claude Code (vault) | Claude Code's Node runtime ignores the system CA store, so it won't trust the CubeEgress CA | The example sets `NODE_EXTRA_CA_CERTS`; override with `CLAUDE_NODE_EXTRA_CA_CERTS` if the CA lives elsewhere |
