...
- The legacy background score refresher for `multi_factor_weighted_average` starts whenever that config section exists, but stays inert until some compiled pipeline (the legacy fallback included) actually binds the scorer. The binding check runs on every tick, so a hot reload that binds or drops the scorer takes effect without a restart, and a configured-but-unbound deployment pays only one idle goroutine instead of a fleet-wide node-score write every `ScoreInterval`.
