...
- Scores default to `default-score`, which substitutes the plugin's `default_score` (itself defaulting to 0) after a transport/invocation failure; `fail-closed` is also available. This is deliberately the opposite direction from filters — health fails closed, quality fails open — and it is silent except for one warning log: every candidate receives the same constant score, the failed plugin's ranking contribution vanishes, and when it is the only scorer the ordering degenerates to candidate order. Deployments that depend on ranking quality should set `failure.score: fail-closed` explicitly. Defect-class failures are never substituted: a panic or nil binding, and validation-class failures (nil, empty-id, duplicate, or non-candidate entries, NaN/Inf or out-of-[0,100] scores, partial coverage), mean the plugin is defective and fail closed under every score policy, `default-score` included. A built-in go scorer's global `Disable()` switch is honored for profile-referenced scorers too — a disabled scorer is skipped like on the legacy path (the expr/gRPC plugin selectors report `Disable()==false` unconditionally, so their bindings are unaffected).
...
Configuration is compiled as one unit at startup or during a hot reload. If a plugin name, route, expression, weight, selection method, or failure policy is invalid, the new Profile set is not activated and the scheduler continues using the previous complete pipeline. Reloads that leave the `scheduler` section byte-identical skip the recompile entirely — external plugin sockets are not re-dialed, and a momentarily-unreachable plugin cannot turn an unrelated reload into a rejected one.
...
- Legacy `enable_filters` / `enable_scorers` entries that are not registered plugins now abort CubeMaster startup with an error naming the offending entry; previously they were silently skipped. Remove stale entries from the config or register the plugin before starting the new version. On the scorer side this presupposes `score.resource_weights` is configured — the legacy scorer list is only compiled under that condition (matching the pre-plugin behavior), so `enable_scorers` entries without `resource_weights` are never compiled and never rejected.
...
- The legacy background score refresher for `multi_factor_weighted_average` starts whenever that config section exists, and runs while some compiled pipeline (the legacy fallback included) binds the scorer **or** the legacy `enable_scorers` condition holds (`score.resource_weights` set and `enable_scorers` non-empty — the pre-plugin activation condition, kept so the `node.Score` operator endpoints stay fed in existing deployments). Otherwise it stays inert. The check runs on every tick, so a hot reload that binds or drops the scorer takes effect without a restart, and a configured-but-unbound deployment pays only one idle goroutine instead of a fleet-wide node-score write every `ScoreInterval`.
...
- The legacy `enable_filters` pipeline now fails a request when a filter returns an error; only an emptied candidate set (`SelectNodesNoRes`) still falls through to `BackoffSelect` (the preFilter stage keeps the old any-error rescue). Previously any filter error — including a defective or panicking filter — was silently rescued by the relaxed-pool random pick, which masked broken filters as random placement. Fix or remove an erroring filter; do not rely on error rescue.
- The legacy template-request backoff exemption — a template request filtered to zero while `template_locality` is enabled fails instead of backing off — applies only to the legacy pipeline. Custom Profiles keep `template_locality` as a mandatory guard (which still fails fast), but a *prefilter* empty result (e.g. the affinity `NodeSelector` matching nothing) on a `no_candidate: backoff` profile falls through to the relaxed pool, and `backoffSelectWithPipeline` re-runs the mandatory guards without re-applying the prefilter's affinity matching — an affinity constraint can therefore be silently relaxed for template requests on Profile pipelines. Route template traffic to a `no_candidate: fail` profile if you rely on the old fail-fast behavior.
...
- Built-in `go` resource filters (`cpu`, `mem`, `disk`) error out on a request that carries no matching resource spec, exactly as the legacy pipeline did (it ran them unconditionally). A Profile that can receive spec-less requests — e.g. the restore-placement path, which passes an empty spec — must not list them as guards or filters. Note that a `default: true` profile cannot opt out: the mandatory guards always run, so spec-less unmatched traffic routed to the default profile can hard-fail with no backoff escape on a packed cluster; compilation logs a RISK warning when a default profile is present.
