...
describe("Sandbox.fork", () => {
  ...
  it("rejects invalid timeoutMs", async () => {
    const sb = await createSandbox();
    await expect(sb.fork({ timeoutMs: -5 })).rejects.toThrow(/timeoutMs/);
  ...
  it("sends NEVER_TIMEOUT as timeout: -1", async () => {
    const sb = await createSandbox();
    let requestBody: string | undefined;
    setHandler((req) => {
      if (req.method === "POST" && req.pathname === forkPath) {
        ...
        return {
          status: 201,
          json: [{ sandbox: { ...SANDBOX_DATA, sandboxID: "sb-fork-t" } }],
        };
      }
      return { status: 500, json: { message: "unexpected" } };
    });
    await sb.fork({ count: 1, timeoutMs: NEVER_TIMEOUT });
    expect(JSON.parse(requestBody ?? "{}")).toEqual({ count: 1, timeout: -1 });
    sb.close();
  });
