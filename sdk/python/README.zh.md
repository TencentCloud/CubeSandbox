...
`Sandbox.create(volume_mounts=[...])` 挂载进沙箱。数据可跨沙箱重启保留，
也可在多个沙箱之间共享。
...
Volume.delete(vol.volume_id)   # 先杀掉所有挂载它的沙箱 —— delete 不会自动 detach
```
...
| 方法 | 说明 |
|---|---|
| `Volume.create(name=None, *, driver=None, config=None)` | `POST /volumes` — 创建卷；省略 `driver` 即 e2b 兼容（后端第一个插件），或传非空 `driver`（如 `"cos"`）绑定插件 → `VolumeInfo` |
| `Volume.list(config)` | `GET /volumes` — 列出卷 → `list[VolumeInfo]`（不含 token） |
| `Volume.get(volume_id, *, config)` | `GET /volumes/:id` — 查询单个卷（含 token） |
| `Volume.delete(volume_id, *, config)` | `DELETE /volumes/:id` — 删除卷 |

通过 `Sandbox.create(volume_mounts=[VolumeMount(name, path)])` 将卷挂载进
沙箱。`VolumeInfo` 暴露 `.volume_id`、`.name`、`.token`。完整参考：
[`docs/volume.zh.md`](docs/volume.zh.md)。
