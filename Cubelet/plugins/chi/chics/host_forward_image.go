...
import (
	...
	"github.com/tencentcloud/CubeSandbox/proto/types/v1"
)
	...
	if err != nil {
		if errdefs.IsNotFound(err) {
			...
			if forwardRequest.GetAuth() != nil {
				ctx = constants.WithImageCredentials(ctx, critypes.AuthConfigToCRI(forwardRequest.GetAuth()))
			}
			...
			credentials := func(host string) (string, string, error) {
				...
				return cubeimages.ParseAuth(critypes.AuthConfigToCRI(forwardRequest.GetAuth()), host)
			}
