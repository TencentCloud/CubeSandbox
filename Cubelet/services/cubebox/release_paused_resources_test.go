...
func TestPausedReserveFactor(t *testing.T) {
	// releaseRatio 0 reserves everything; 1 releases everything; values in
	// between reserve (1-r); out-of-range values are clamped.
	assert.Equal(t, 1.0, pausedReserveFactor(0), "release nothing => reserve all")
	assert.Equal(t, 0.0, pausedReserveFactor(1), "release all => reserve none")
	assert.Equal(t, 0.75, pausedReserveFactor(0.25))
	assert.Equal(t, 1.0, pausedReserveFactor(-1), "negative ratio clamps to 0 release")
	assert.Equal(t, 0.0, pausedReserveFactor(2), "ratio above 1 clamps to full release")
}
