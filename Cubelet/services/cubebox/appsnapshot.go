...
func (s *service) AppSnapshot(ctx context.Context, req *cubebox.AppSnapshotRequest) (*cubebox.AppSnapshotResponse, error) {
	...
	if !keepPaused {
		stepLog.Warn(legacyRuntimeSnapshotWarning)
	}
	snapshotErr, rootfsErr, resumeErr := runAppSnapshotWithRootfs(keepPaused, func() error {
		return s.executeCubeRuntimeSnapshotWithPause(frozenCtx, sandboxID, spec, layout.MetaWork, memoryObject.DevPath, snapshotTypeFull, keepPaused)
	}, func() error {
		var commitErr error
...
func (s *service) executeCubeRuntimeSnapshotPaused(ctx context.Context, sandboxID string, spec *CubeboxSnapshotSpec, snapshotPath, memoryVol, snapshotType string) error {
	return s.executeCubeRuntimeSnapshotWithPause(ctx, sandboxID, spec, snapshotPath, memoryVol, snapshotType, true)
}
