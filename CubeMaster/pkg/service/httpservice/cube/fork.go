...
func forkSandboxFromSnapshot(
	...
	// Per-slot requestIDs (parent-<idx>) keep logs/audit distinguishable.
	sem := make(chan struct{}, forkCreateConcurrencyLimit)
	for i := 0; i < count; i++ {
		wg.Add(1)
		go func(idx int) {
			defer wg.Done()
			// A panic degrades to one error entry, not a process crash
			// (recover only works on the panicking goroutine).
			defer func() {
				if r := recover(); r != nil {
					CubeLog.WithContext(ctx).Errorf("fork derive %s-%d panic: %v", requestID, idx, r)
					results[idx] = &forkSandboxResult{Ret: forkRet(
						errorcode.ErrorCode_MasterInternalError,
						fmt.Sprintf("derive panic: %v", r),
					)}
				}
			}()
			sem <- struct{}{}
			...
			sb, ret := singleForkDerive(subCtx, fmt.Sprintf("%s-%d", requestID, idx), snapshotID, timeout, envVars)
...
func forkSandboxFromSource(
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				CubeLog.WithContext(ctx).Errorf("fork snapshot reclaim panic: %v", r)
			}
		}()
