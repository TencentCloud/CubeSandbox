...
// TestForkDeriveUsesPerSlotRequestIDs verifies each derivation carries its
// own requestID (parent-<idx>), keeping logs/audit distinguishable.
func TestForkDeriveUsesPerSlotRequestIDs(t *testing.T) {
	var reclaim forkReclaimRecord
	stubForkCreateSeams(t, &reclaim)
	idsCh := make(chan string, 3)
	forkCreateSandboxRunFn = func(_ context.Context, req *types.CreateCubeSandboxReq) *types.CreateCubeSandboxRes {
		idsCh <- req.Request.RequestID
		return &types.CreateCubeSandboxRes{
			Ret:       &types.Ret{RetCode: int(errorcode.ErrorCode_Success)},
			SandboxID: "sb-fork-" + req.Request.RequestID,
		}
	}

	ctx := context.Background()
	resp := &forkSandboxResponse{Res: &types.Res{Ret: &types.Ret{RetCode: int(errorcode.ErrorCode_Success)}}}
	err := forkSandboxFromSource(ctx, "req-fork", testForkSandboxID, 3, nil, resp)
	...
	assert.Equal(t, map[string]bool{"req-fork-0": true, "req-fork-1": true, "req-fork-2": true}, ids)
	waitReclaimed(t, &reclaim)
}

// TestForkDerivePanicDegradesToPerForkError verifies a panicking derivation
// becomes one error entry instead of crashing the process.
func TestForkDerivePanicDegradesToPerForkError(t *testing.T) {
	var reclaim forkReclaimRecord
	stubForkCreateSeams(t, &reclaim)
	forkCreateSandboxRunFn = func(ctx context.Context, req *types.CreateCubeSandboxReq) *types.CreateCubeSandboxRes {
		if strings.HasSuffix(req.Request.RequestID, "-0") {
			panic("boom")
		}
		return &types.CreateCubeSandboxRes{
			Ret:       &types.Ret{RetCode: int(errorcode.ErrorCode_Success)},
	...
	require.NoError(t, err)
	require.Len(t, resp.Results, 2)
	// Slot 0 panicked: one error entry, not a crash.
	assert.Nil(t, resp.Results[0].Sandbox)
	if assert.NotNil(t, resp.Results[0].Ret) {
		assert.Equal(t, int(errorcode.ErrorCode_MasterInternalError), resp.Results[0].Ret.RetCode)
		assert.Contains(t, resp.Results[0].Ret.RetMsg, "derive panic")
	}
	// Slot 1 is unaffected.
	assert.NotNil(t, resp.Results[1].Sandbox)
	assert.Nil(t, resp.Results[1].Ret)
	waitReclaimed(t, &reclaim)
}
