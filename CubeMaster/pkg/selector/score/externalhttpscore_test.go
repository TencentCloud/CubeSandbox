...
func TestExternalHTTPScoreRegistryFactory(t *testing.T) {
	reg, ok := scores[externalHTTPScoreName]
	if !ok || reg.new == nil {
		t.Fatal("external_http_score missing from package registry")
	}
	if reg.requiresPluginConf == nil {
		t.Fatal("external_http_score must set requiresPluginConf")
	}
	if !reg.requiresPluginConf(config.ScorePluginConf{ExternalHTTPScore: &config.ExternalHTTPScore{}}) {
		t.Fatal("requiresPluginConf=false with present plugin_conf block")
	}
	if reg.requiresPluginConf(config.ScorePluginConf{}) {
		t.Fatal("requiresPluginConf=true with missing plugin_conf block")
	}
	fn := reflect.ValueOf(reg.new)
