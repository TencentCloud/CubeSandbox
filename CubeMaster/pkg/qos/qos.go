...
var inferTemplateDefaults = func(TemplateResources) *Config {
	// Reserved for the future CPU/memory-derived platform profile. Returning
	// nil intentionally keeps QoS opt-in until that mapping is defined.
	return nil
}
...
func mergeExplicit(resolved, explicit *Config) {
	...
	if explicit.Network != nil {
		...
		// The flags preserve field presence from JSON; the value checks also
		// support callers that construct Config programmatically.
...
func parseBlockIORate(bucket *blockIOTokenBucket, unit uint64, name string) (uint32, error) {
	...
	numerator, numeratorOK := multiplyUint64(bucket.Size, 1000)
	denominator, denominatorOK := multiplyUint64(bucket.RefillTime, unit)
...
func HasAppliedAnnotation(networkRaw, blockIORaw string) bool {
	networkRaw = strings.TrimSpace(networkRaw)
	blockIORaw = strings.TrimSpace(blockIORaw)
	return (networkRaw != "" && networkRaw != "{}") ||
		(blockIORaw != "" && blockIORaw != "{}")
}
...
func ParseAnnotation(raw string) (*Config, error) {
	...
	if payload.Version == AnnotationVersion {
...
func parseBandwidthBucket(bucket annotationLimiter) (uint32, error) {
	...
	numerator, numeratorOK := multiplyUint64(bucket.Size, 8_000)
	denominator, denominatorOK := multiplyUint64(bucket.RefillTime, 1_000_000)
	if !numeratorOK || !denominatorOK {
...
func parsePacketsBucket(bucket annotationLimiter) (uint32, error) {
	...
	numerator, ok := multiplyUint64(bucket.Size, 1000)
	if !ok {
...
func multiplyUint64(value, factor uint64) (uint64, bool) {
...
func decodeStrictJSON(data []byte, out any) error {
	...
	return decodeJSONWithDecoder(decoder, out)
}

func decodeJSON(data []byte, out any) error {
	return decodeJSONWithDecoder(json.NewDecoder(bytes.NewReader(data)), out)
}

func decodeJSONWithDecoder(decoder *json.Decoder, out any) error {
