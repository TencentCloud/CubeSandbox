...
func TestParseBlockIOAnnotationAllowsLegacyUnknownFields(t *testing.T) {
	blockIO, err := ParseBlockIOAnnotation(`{"bandwidth":{"size":67108864,"refill_time":1000,"burst":1}}`)
	if err != nil {
		t.Fatalf("ParseBlockIOAnnotation error=%v", err)
	}
	if blockIO == nil || blockIO.ThroughputMiBps != 64 {
		t.Fatalf("block io config=%+v, want 64 MiB/s", blockIO)
	}
}
...
func TestParseAnnotationsRejectsOverflowingTokenBuckets(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "network bandwidth size",
			network: `{"Qos":{"BandWidth":{"Size":` + maxUint64 + `,"RefillTime":100},"OPS":{}},"Version":1}`,
		},
		{
			name:    "network bandwidth refill time",
			network: `{"Qos":{"BandWidth":{"Size":1,"RefillTime":` + maxUint64 + `},"OPS":{}},"Version":1}`,
		},
		{
			name:    "network packets size",
			network: `{"Qos":{"BandWidth":{},"OPS":{"Size":` + maxUint64 + `,"RefillTime":1000}},"Version":1}`,
		},
		{
			name:    "block throughput size",
			blockIO: `{"bandwidth":{"size":` + maxUint64 + `,"refill_time":1000}}`,
		},
		{
			name:    "block throughput refill time",
			blockIO: `{"bandwidth":{"size":1,"refill_time":` + maxUint64 + `}}`,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := ParseAnnotations(tt.network, tt.blockIO)
			if err == nil || !strings.Contains(err.Error(), "outside the supported") {
				t.Fatalf("ParseAnnotations error=%v, want supported-range error", err)
			}
		})
	}
}
...
func TestParseAnnotationAcceptsLegacyVersionZero(t *testing.T) {
	cfg, err := ParseAnnotation(`{"Mode":"legacy","Extension":true,"Qos":{"BandWidth":{"Size":1250000,"RefillTime":100}}}`)
	if err != nil {
		t.Fatalf("ParseAnnotation error=%v", err)
	}
...
func TestParseAnnotationsTreatsEmptyNetworkObjectAsDisabled(t *testing.T) {
	cfg, err := ParseAnnotations(
		`{}`,
		`{"bandwidth":{"size":67108864,"refill_time":1000},"ops":{"size":1000,"refill_time":1000}}`,
	)
	if err != nil {
		t.Fatalf("ParseAnnotations error=%v", err)
	}
	if cfg == nil || cfg.Network != nil || cfg.BlockIO == nil || cfg.BlockIO.ThroughputMiBps != 64 || cfg.BlockIO.IOPS != 1000 {
		t.Fatalf("config=%+v, want block-only QoS", cfg)
	}
}
...
func TestParseAnnotationRejectsVersionOneExtensionsAndMode(t *testing.T) {
	for _, raw := range []string{
		`{"Extension":true,"Qos":{"BandWidth":{"Size":1250000,"RefillTime":100}},"Version":1}`,
		`{"Mode":"legacy","Qos":{"BandWidth":{"Size":1250000,"RefillTime":100}},"Version":1}`,
	} {
		if _, err := ParseAnnotation(raw); err == nil {
			t.Fatalf("expected version 1 annotation rejection for %s", raw)
		}
	}
}
