...
func Test_injectEnvdIntoRootfs_optIn_copiesBinary(t *testing.T) {
	...
	dst := filepath.Join(rootfs, envdInImagePath)
...
func Test_injectEnvdIntoRootfs_disabled_leavesRootfsUntouched(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			assert.Empty(t, sha)
			_, err = os.Stat(filepath.Join(rootfs, envdInImagePath))
...
func Test_injectEnvdIntoRootfs_missingBinary_failsLoud(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Empty(t, sha)
	_, statErr := os.Stat(filepath.Join(rootfs, envdInImagePath))
...
func Test_buildTemplateSpecFingerprint_envdInfluence(t *testing.T) {
	...
	fpEnvdV1 := buildTemplateSpecFingerprint(envdReq, "sha256:src")
	...
	fpEnvdV2 := buildTemplateSpecFingerprint(envdReq, "sha256:src")
