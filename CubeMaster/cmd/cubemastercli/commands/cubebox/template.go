...
var TemplateCreateFromImageCommand = cli.Command{
	...
	Flags: []cli.Flag{
		...
		cli.StringFlag{Name: "name", Usage: "optional display name for the template"},
...
var TemplateListCommand = cli.Command{
	...
	Action: func(c *cli.Context) error {
		...
		tabHeader := "TEMPLATE_ID\tNAME\tSTATUS\tCREATED_AT\tIMAGE_INFO"
		if wideOutput {
			tabHeader = "TEMPLATE_ID\tNAME\tSTATUS\tLAST_ERROR\tCREATED_AT\tIMAGE_INFO"
		}
		...
		for _, item := range rsp.Data {
			...
			if wideOutput {
				fmt.Fprintf(w, "%s\t%s\t%s\t%s\t%s\t%s\n",
					item.TemplateID, name, item.Status, item.LastError, item.CreatedAt, item.ImageInfo)
				continue
			}
			fmt.Fprintf(w, "%s\t%s\t%s\t%s\t%s\n",
				item.TemplateID, name, item.Status, item.CreatedAt, item.ImageInfo)
		}
		return w.Flush()
	},
}
